...
describe('AutopilotControlScreen', () => {
  ...
  describe('UI Layout & Design', () => {
    it('renders main control interface with proper elements', () => {
      const { getByText, queryByText } = render(<AutopilotControlScreen />);
    ...
    it('shows engage button when autopilot is not engaged', () => {
      const { getByText } = render(<AutopilotControlScreen />);
  ...
  describe('Touch Interactions & Haptics', () => {
    it('triggers haptic feedback on engage button press', async () => {
      ...
      fireEvent.press(getByText('ENGAGE AUTOPILOT'));
    ...
    it('triggers haptic feedback on emergency disengage', async () => {
      ...
      expect(Vibration.vibrate).toHaveBeenCalledWith([100, 50, 100]);
    });
    ...
    it('has minimum 44pt touch targets for accessibility', () => {
      ...
      const engageButton = getByText('ENGAGE AUTOPILOT');
  ...
  describe('Safety Features', () => {
    it('shows confirmation modal for engagement', async () => {
      ...
      // Initially no modal
      expect(queryByText('ENGAGE AUTOPILOT')).toBeTruthy(); // Button exists
      expect(queryByText('Are you ready to engage autopilot on heading 180°?')).toBeFalsy();
      ...
      // Modal should appear
      await waitFor(() => {
        expect(getByText('Are you ready to engage autopilot on heading 180°?')).toBeTruthy();
    ...
    it('confirms engagement through modal', async () => {
      ...
      await waitFor(() => {
        expect(getByText('ENGAGE')).toBeTruthy(); // Command feedback
      });
    });
    ...
    it('emergency disengage requires no confirmation', async () => {
      ...
      // Should immediately show command feedback without modal
      await waitFor(() => {
        expect(getByText('DISENGAGE')).toBeTruthy();
      });
    });
  ...
  describe('Heading Adjustments', () => {
    ...
    it('allows +/-1° heading adjustments when engaged', async () => {
      ...
      await waitFor(() => {
        expect(getByText('ADJUST +1°')).toBeTruthy();
      });
    });
    ...
    it('allows +/-10° heading adjustments when engaged', async () => {
      ...
      await waitFor(() => {
        expect(getByText('ADJUST -10°')).toBeTruthy();
      });
    });
  ...
  describe('Visual Feedback', () => {
    it('shows correct status colors for different states', () => {
      // Test OFF state
      let { getByText } = render(<AutopilotControlScreen />);
      expect(getByText('OFF')).toBeTruthy();
      ...
      const { rerender } = render(<AutopilotControlScreen />);
      expect(getByText('ENGAGED')).toBeTruthy();
    });
  ...
  describe('Responsive Design', () => {
    ...
    it('handles landscape orientation', () => {
      ...
      const { getByText } = render(<AutopilotControlScreen />);
      expect(getByText('AUTOPILOT')).toBeTruthy(); // Different title in landscape
    });
  });
  ...
  describe('Integration', () => {
    it('calls onClose when close button is pressed', () => {
      ...
      const { getByText } = render(<AutopilotControlScreen onClose={mockOnClose} />);
