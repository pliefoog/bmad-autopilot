...
const App = () => {
  ...
  // Uses WidgetDetectionService for clean, maintainable widget lifecycle management
  useEffect(() => {
    ...
    try {
      // Import and use WidgetDetectionService for centralized detection logic
      import('../services/widgetDetectionService').then(({ WidgetDetectionService }) => {
        const { toCreate } = WidgetDetectionService.detectRequiredWidgets(
          nmeaData,
          dashboard.widgets
        );
        
        // Create detected widgets
        toCreate.forEach(({ id, type, instance, title }) => {
          if (instance !== undefined) {
            // Multi-instance widget (engine, battery, tank, temperature)
            console.log(`[App] ➕ Creating ${type} widget: ${id}`);
            createInstanceWidget(
              instance.toString(),
              type as 'engine' | 'battery' | 'tank',
              title || `${type} ${instance + 1}`,
              { x: 0, y: 0 }
            );
          } else {
            // Single-instance widget (depth, gps, speed, wind, compass)
            console.log(`[App] ➕ Creating ${type} widget: ${id}`);
            addWidget(id, { x: 0, y: 0 });
          }
        });
        
        if (toCreate.length > 0) {
          console.log(`[App] ✅ Created ${toCreate.length} new widgets`);
        }
      }).catch(error => {
        console.error('[App] Failed to detect widgets:', error);
      });
    } catch (error) {
      console.error('[App] Error in widget lifecycle management:', error);
    }
