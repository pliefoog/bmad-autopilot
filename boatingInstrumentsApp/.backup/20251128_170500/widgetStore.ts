...
const defaultDashboard: DashboardConfig = {
  ...
  widgets: [
    // System widgets - always present
    {
      ...
      enabled: true,
      order: 0,
      isPinned: true, // Permanent widgets are always pinned
      createdAt: Date.now(),
    },
    // All other widgets are dynamically detected from NMEA messages:
  ...
  persist(
    (set, get) => ({
      ...
      initializeWidgetStatesOnAppStart: () => {
        ...
        // 🔧 PERMANENT WIDGETS: Ensure all permanent widgets from registry are present
        if (dashboard) {
          // Import WidgetRegistry dynamically to get permanent widgets
          import('../widgets/WidgetRegistry').then(({ WidgetRegistry }) => {
            const permanentWidgets = WidgetRegistry.getPermanentWidgets();
            const existingWidgetIds = new Set(dashboard!.widgets.map(w => w.id));
            const missingPermanent = permanentWidgets.filter(w => !existingWidgetIds.has(w.meta.id));
            
            if (missingPermanent.length > 0) {
              console.log('[WidgetStore] Missing permanent widgets detected:', missingPermanent.map(w => w.meta.id));
              console.log('[WidgetStore] Adding missing permanent widgets to dashboard');
              
              // Create widget configs for missing permanent widgets
              const newPermanentWidgets = missingPermanent.map((widgetMeta, index) => ({
                id: widgetMeta.meta.id,
                type: widgetMeta.meta.id,
                title: widgetMeta.meta.title,
                settings: {},
                layout: {
                  id: widgetMeta.meta.id,
                  x: (dashboard!.widgets.length + index) % 6 * 2,
                  y: Math.floor((dashboard!.widgets.length + index) / 6) * 2,
                  width: 2,
                  height: 2,
                  visible: true,
                },
                enabled: true,
                order: dashboard!.widgets.length + index,
                isPinned: true, // Permanent widgets are always pinned
                createdAt: Date.now(),
              }));
              
              // Add missing permanent widgets
              const updatedWidgets = [...dashboard!.widgets, ...newPermanentWidgets];
              ...
              console.log('[WidgetStore] Permanent widgets added. Total widgets now:', dashboard?.widgets.length);
            }
          }).catch(error => {
            console.error('[WidgetStore] Failed to load WidgetRegistry:', error);
          });
        }
        ...
        set((currentState) => ({
          dashboards: currentState.dashboards.map((dashboard) => ({
            ...
            widgets: dashboard.widgets.filter((widget) => {
              // Check if widget is permanent via registry
              let isPermanent = false;
              try {
                // Import WidgetRegistry to check if widget is permanent
                import('../widgets/WidgetRegistry').then(({ WidgetRegistry }) => {
                  isPermanent = WidgetRegistry.isPermanent(widget.id);
                });
              } catch (error) {
                // Fallback: check specific IDs if registry not available
                isPermanent = widget.id === 'theme' || widget.type === 'theme';
              }
              
              // Always keep permanent widgets
              if (isPermanent || widget.isPinned) {
                return true;
              }
          ...
          selectedWidgets: currentState.selectedWidgets.filter((widgetType) => {
            ...
            return currentState.dashboards.some((dashboard) =>
              dashboard.widgets.some((widget) => {
                const isPermanentType = widget.id === 'theme' || widget.type === 'theme';
                return widget.type === widgetType && 
                  (isPermanentType || widget.isPinned ||
