...
class MarineDomainValidator {
  ...
  validateNavigationAccuracy(navigationData) {
    ...
    if (
      navigationData.referenceLatitude !== undefined &&
      navigationData.referenceLongitude !== undefined
    ) {
      const distance = this.calculateDistanceNM(
        navigationData.latitude,
        navigationData.longitude,
        navigationData.referenceLatitude,
        navigationData.referenceLongitude,
      );
    ...
    if (navigationData.heading !== undefined && navigationData.referenceHeading !== undefined) {
      const headingError = this.calculateHeadingError(
        navigationData.heading,
        navigationData.referenceHeading,
      );
    ...
    if (navigationData.course !== undefined && navigationData.referenceCourse !== undefined) {
      const courseError = this.calculateHeadingError(
        navigationData.course,
        navigationData.referenceCourse,
      );
  ...
  validateEnvironmentalAccuracy(environmentalData) {
    ...
    if (
      environmentalData.windDirection !== undefined &&
      environmentalData.referenceWindDirection !== undefined
    ) {
    ...
    if (
      environmentalData.windSpeed !== undefined &&
      environmentalData.referenceWindSpeed !== undefined
    ) {
      const windSpeedError = Math.abs(
        environmentalData.windSpeed - environmentalData.referenceWindSpeed,
      );
  ...
  validateEngineAccuracy(engineData) {
    ...
    if (engineData.rpm !== undefined && engineData.referenceRpm !== undefined) {
      const rpmErrorPercent =
        Math.abs((engineData.rpm - engineData.referenceRpm) / engineData.referenceRpm) * 100;
