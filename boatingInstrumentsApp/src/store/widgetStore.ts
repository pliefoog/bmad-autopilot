...
interface WidgetState {
  ...
  // Pagination state (Story 6.11 - AC 9: Page State Persistence)
  currentPage: number;
  totalPages: number;
  pageWidgets: Record<number, string[]>; // Map of page index to widget IDs
  maxWidgetsPerPage: number;
  isAnimatingPageTransition: boolean;
}
...
interface WidgetActions {
  ...
  // Pagination methods (Story 6.11)
  setCurrentPage: (page: number) => void;
  navigateToPage: (page: number) => void;
  navigateToNextPage: () => void;
  navigateToPreviousPage: () => void;
  addWidgetToOptimalPage: (widgetType: string) => void;
  recalculatePages: (maxWidgetsPerPage: number) => void;
  getWidgetsForPage: (pageIndex: number) => string[];
  moveWidgetToPage: (widgetId: string, targetPage: number) => void;
}
...
export const useWidgetStore = create<WidgetStore>()(
  persist(
    (set, get) => ({
      ...
      gridVisible: false,
      widgetExpanded: {},
      // Pagination state (Story 6.11)
      currentPage: 0,
      totalPages: 1,
      pageWidgets: { 0: ['depth', 'speed', 'wind', 'gps', 'compass'] },
      maxWidgetsPerPage: 6, // Default for tablet
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Pagination state persistence (AC 9: Page State Persistence)
        currentPage: state.currentPage,
        pageWidgets: state.pageWidgets,
        maxWidgetsPerPage: state.maxWidgetsPerPage,
      }),
    }
  )
);