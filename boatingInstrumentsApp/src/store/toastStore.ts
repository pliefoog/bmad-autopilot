  ...
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      addToast: (toastData) => {
        ...
        const recentDuplicate = stateForDuplicateCheck.toasts.find(
          (existing) =>
        ...
        set((state) => {
          ...
          const MAX_TOASTS = 5;
          const limitedToasts =
            newToasts.length > MAX_TOASTS ? newToasts.slice(-MAX_TOASTS) : newToasts;
    ...
    { name: 'Toast Store', enabled: __DEV__ },
  ),
);
