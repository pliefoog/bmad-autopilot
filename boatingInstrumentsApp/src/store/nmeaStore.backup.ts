...
export const useNmeaStore = create<NmeaStore>()(
  devtools(
    (set, get) => ({
      ...
      updateSensorData: <T extends SensorType>(
        ...
        if (lastUpdate && now - lastUpdate < throttleMs) {
          ...
          if (sensorType === 'speed') {
            log.speed('Throttled update', () => ({
              instance,
              timeSinceLastMs: now - lastUpdate,
              throttleMs,
            }));
          }
          return;
        ...
        if (sensorType === 'speed') {
          log.storeUpdate('Store update', () => ({ sensorType, instance, data }));
        }
        ...
        set((state) => {
          ...
          if (currentSensorData && Object.keys(data).length > 0) {
            ...
            if (!hasChanges) {
              ...
              if (sensorType === 'speed') {
                log.storeUpdate('NO CHANGES: Rejected update', () => ({
                  sensorType,
                  instance,
          ...
          // Debug: Log what data we're about to enrich for depth
          if (sensorType === 'depth') {
            log.depth('Pre-enrichment data', () => ({
              instance,
              currentDepth: (currentSensorData as any)?.depth,
              finalDataDepth: (finalData as any)?.depth,
              finalDataKeys: Object.keys(finalData),
              mergedDepth: { ...currentSensorData, ...finalData }.depth,
            }));
          }
          ...
          if (trackableValue !== null && history) {
            ...
            if (sensorType === 'depth' && Math.random() < 0.05) {
              ...
              log.storeUpdate('Depth history', () => ({
                value: trackableValue.toFixed(2),
                totalPoints: stats.totalCount,
              }));
            }
        ...
        if (isNewInstance) {
          log.storeInit(`🆕 NEW SENSOR DETECTED: ${sensorType}[${instance}]`);
          ...
          log.storeInit(`🔍 Checking thresholds for ${sensorType}[${instance}]:`, () => ({
          ...
          if (!sensor?.alarmThresholds) {
            ...
            log.storeInit(`📋 Getting alarm defaults for ${sensorType}[${instance}]:`, () => ({
              context,
              location,
            }));
            ...
            if (defaults) {
              ...
              log.storeInit(`💾 Applying defaults for ${sensorType}[${instance}]:`, () => ({
                defaults,
                shouldAutoEnable,
              }));
              ...
              log.storeInit(
                `✅ Auto-initialized ${
                  shouldAutoEnable ? 'ENABLED' : 'disabled'
                } default thresholds for ${sensorType}[${instance}] at ${location || 'default'}`,
              );
            } else {
              log.storeInit(
                `⚠️ No defaults available for ${sensorType}[${instance}] at ${
                  location || 'default'
                }`,
              );
            }
          } else {
            log.storeInit(`⏭️ Skipping ${sensorType}[${instance}] - already has thresholds`);
          }
        }
      ...
      getSensorHistory: <T extends SensorType>(
        ...
        if (sensorType === 'depth' && Math.random() < 0.1) {
          log.storeUpdate('getSensorHistory', () => ({
            sensorType,
            instance,
            points: data.length,
          }));
    ...
    { name: 'NMEA Store', enabled: __DEV__ },
  ),
);
