...
export function useNMEAData(options: UseNMEADataOptions = {}): UseNMEADataReturn {
  ...
  const assessDataQuality = useCallback((data: Partial<NmeaData>): { quality: DataQuality; metrics: QualityMetrics } => {
    const now = Date.now();
    ...
    const timeliness = lastUpdate ? Math.max(0, 100 - ((now - lastUpdate) / opts.staleDataThreshold) * 100) : 0;
  ...
  useEffect(() => {
    const now = Date.now();
    const hasNewData = Object.keys(filteredData).some(key => 
      filteredData[key as keyof NmeaData] != null
    );
