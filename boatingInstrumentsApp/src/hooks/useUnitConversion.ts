...
export interface UseUnitConversionReturn {
  ...
  convertToPreferred: (
    value: number,
    fromUnit: string,
  ) => { value: number; unit: UnitDefinition } | null;
  convertFromBase: (
    value: number,
    category: string,
    targetUnit?: string,
  ) => { value: number; unit: UnitDefinition };

  // Formatting functions
  format: (
    value: number,
    unit: string,
    options?: { precision?: number; showSymbol?: boolean; formatId?: string },
  ) => string;
  formatWithPreferred: (
    value: number,
    fromUnit: string,
    options?: { showBoth?: boolean; precision?: number; formatId?: string },
  ) => string;
  ...
  getFormattedDateTimeWithTimezone: (date: Date) => {
    date: string;
    time: string;
    timezone: string;
  };
  ...
  ) => Array<{ value: number; unit: UnitDefinition }>;
...
function formatDate(date: Date, formatId: string, timezone: string = 'utc'): string {
  ...
  switch (formatId) {
    ...
    case 'us_date':
      return `${dayAbbr} ${(workingDate.getMonth() + 1).toString().padStart(2, '0')}/${workingDate
        .getDate()
        .toString()
        .padStart(2, '0')}/${workingDate.getFullYear()}`; // DDD MM/DD/YYYY

    case 'eu_date':
      return `${dayAbbr} ${workingDate.getDate().toString().padStart(2, '0')}.${(
        workingDate.getMonth() + 1
      )
        .toString()
        .padStart(2, '0')}.${workingDate.getFullYear()}`; // DDD DD.MM.YYYY

    case 'uk_date':
      return `${dayAbbr} ${workingDate.getDate()} ${
        monthNames[workingDate.getMonth()]
      } ${workingDate.getFullYear()}`; // DDD DD MMM YYYY

    case 'nautical_date':
      return `${dayAbbr}, ${
        monthNames[workingDate.getMonth()]
      } ${workingDate.getDate()}, ${workingDate.getFullYear()}`; // DDD, MMM DD, YYYY
...
function formatTime(date: Date, formatId: string, timezone: string = 'utc'): string {
  ...
  switch (formatId) {
    ...
    case 'time_12h_full':
      ...
      return `${hours12Full.toString().padStart(2, '0')}:${minutes
        .toString()
...
function getConsistentFormatWidth(
  category: string,
  unitSymbol: string,
): {
  minWidth: number;
  ...
  const categoryContentSamples: Record<
    string,
    {
      maxContent: string;
...
const UNIT_DEFINITIONS: UnitDefinition[] = [
    ...
    displayFormats: [
      {
        ...
        name: 'Scale Value',
        precision: 0,
        formatter: (knotsValue) => knotsToBeaufort(knotsValue).toString(),
        isDefault: true,
      },
      {
        id: 'description',
    ...
    displayFormats: [
      {
        ...
        name: 'DD (6 decimals)',
        precision: 6,
        formatter: (v) => formatDecimalDegrees(v, 6),
        isDefault: true,
      },
      {
        id: 'dd_4',
        name: 'DD (4 decimals)',
        precision: 4,
        formatter: (v) => formatDecimalDegrees(v, 4),
      },
      {
        id: 'dd_2',
        name: 'DD (2 decimals)',
        precision: 2,
        formatter: (v) => formatDecimalDegrees(v, 2),
      },
    ],
  ...
  },
  {
    ...
    displayFormats: [
      {
        id: 'ddm_3',
        name: 'DDM (3 decimal minutes)',
        precision: 3,
        formatter: (v) => formatDegreesMinutes(v),
        isDefault: true,
      },
      {
        id: 'ddm_2',
        name: 'DDM (2 decimal minutes)',
        precision: 2,
        formatter: (v) => formatDegreesMinutes(v),
      },
      {
        id: 'ddm_1',
        name: 'DDM (1 decimal minutes)',
        precision: 1,
        formatter: (v) => formatDegreesMinutes(v),
      },
    ],
  ...
  },
  {
    ...
    displayFormats: [
      {
        id: 'dms_1',
        name: 'DMS (1 decimal seconds)',
        precision: 1,
        formatter: (v) => formatDegreesMinutesSeconds(v),
        isDefault: true,
      },
      {
        id: 'dms_0',
        name: 'DMS (integer seconds)',
        precision: 0,
        formatter: (v) => formatDegreesMinutesSeconds(v),
      },
    ],
  ...
  },
  {
    ...
    displayFormats: [
      {
        id: 'utm_standard',
        name: 'UTM (zone + northing/easting)',
        precision: 0,
  ...
  },
  {
    ...
    category: 'capacity',
    baseUnit: 'ampere_hour',
    conversionFactor: 83.33,
    precision: 2,
  }, // Approx for 12V system
  {
    id: 'watt_hour',
    name: 'Watt Hour',
    symbol: 'Wh',
  ...
  // Flow Rate (NEW - Marine fuel consumption, bilge pumps, etc.)
  {
    ...
    system: 'metric',
    category: 'flow_rate',
    baseUnit: 'liter_per_hour',
    conversionFactor: 1,
    precision: 2,
  },
  {
    id: 'gallon_us_per_hour',
  ...
  {
    id: 'gallon_uk_per_hour',
    name: 'Imperial Gallons per Hour',
    symbol: 'gal/h',
    system: 'imperial',
    category: 'flow_rate',
    baseUnit: 'liter_per_hour',
  ...
  {
    id: 'nautical_date',
    name: 'Nautical Date Format',
    symbol: 'DDD, MMM DD, YYYY',
    system: 'nautical',
    category: 'date',
    baseUnit: 'iso_date',
    conversionFactor: 1,
...
export function useUnitConversion(options: UseUnitConversionOptions = {}): UseUnitConversionReturn {
  ...
  const [marineRegion, setMarineRegionState] = useState<'eu' | 'us' | 'uk' | 'international'>(
    defaultMarineRegion,
  );
    ...
    Object.entries(units).forEach(([storeCategory, storeUnit]) => {
      ...
      if (conversionCategory && unitIdMapping[conversionCategory]) {
        ...
        if (preferredUnitId) {
          ...
          const defaultFormat =
            unit?.marineRegionDefaults?.[marineRegion] ||
  ...
  const setSystem = useCallback(
    (system: UnitSystem) => {
      ...
      const newPreferences: ConversionPreference[] = Object.entries(systemDefaults).map(
        ([category, unitId]) => ({
          category,
  ...
  const setMarineRegion = useCallback(
    (region: 'eu' | 'us' | 'uk' | 'international') => {
      ...
      const updatedPreferences = preferences.map((pref) => {
        ...
        if (unit?.marineRegionDefaults?.[region]) {
          return {
            ...pref,
            preferredFormat: unit.marineRegionDefaults[region],
            marineRegion: region,
          };
        }
  ...
  const setPreferredFormat = useCallback(
    (category: string, unitId: string, formatId: string) => {
      setPreferences((prev) => {
  ...
  const getPreferredFormat = useCallback(
    (category: string, unitId: string): DisplayFormat | undefined => {
      const preference = preferences.find(
        (p) => p.category === category && p.preferredUnit === unitId,
      );
      const unit = findUnit(unitId);
  ...
  const getDefaultFormat = useCallback(
    (unitId: string, region?: 'eu' | 'us' | 'uk' | 'international'): DisplayFormat | undefined => {
      const unit = findUnit(unitId);
  ...
  const getPreferredUnit = useCallback(
    (category: string): UnitDefinition | undefined => {
  ...
  const convert = useCallback(
    (value: number, fromUnit: string, toUnit: string): number | null => {
      ...
      if (fromDef.category !== toDef.category) {
        onError?.(
          `Cannot convert between different categories: ${fromDef.category} and ${toDef.category}`,
        );
        return null;
      }
  ...
  const convertFromBase = useCallback(
    (value: number, category: string, targetUnit?: string) => {
      const unit = targetUnit
        ? availableUnits.find((u) => u.id === targetUnit)
        : getPreferredUnit(category);
      if (!unit) {
  ...
  // Unit utilities
  const getUnitsInCategory = useCallback(
    (category: string) => {
  ...
  const getUnitsBySystem = useCallback(
    (system: UnitSystem) => {
  ...
  const validateConversion = useCallback(
    (fromUnit: string, toUnit: string): { valid: boolean; error?: string } => {
      ...
      if (fromDef.category !== toDef.category) {
        return {
          valid: false,
          error: `Cannot convert between ${fromDef.category} and ${toDef.category}`,
        };
      }
  ...
  const isCompatible = useCallback(
    (unit1: string, unit2: string): boolean => {
  ...
  const bulkConvert = useCallback(
    (values: Array<{ value: number; unit: string }>, targetSystem: UnitSystem) => {
      return values.map(({ value, unit }) => {
        const fromDef = findUnit(unit);
        if (!fromDef)
          return { value, unit: { id: unit, name: unit, symbol: unit } as UnitDefinition };
  ...
  const importPreferences = useCallback(
    (newPreferences: ConversionPreference[]) => {
   ...
   */
  const getPreferredUnitSymbol = useCallback(
  ...
  const convertAndFormat = useCallback(
    (value: number | null | undefined, category: string, fromUnit?: string): string => {
      ...
      if (converted) {
        ...
        if (preferredFormat) {
          ...
          if (!preferredFormat.condition || preferredFormat.condition(converted.value)) {
            ...
            const alternativeFormat = unit?.displayFormats?.find(
              (f) => f.id !== preferredFormat.id && (!f.condition || f.condition(converted.value)),
            );
   ...
   */
  const getFormattedDateTimeWithTimezone = useCallback(
    (
      date: Date,
    ): {
      date: string;
   ...
   */
  const getGpsFormattedDateTime = useCallback(
    (
      date: Date,
    ): {
      date: string;
   ...
   */
  const getShipFormattedDateTime = useCallback(
    (
      date: Date,
    ): {
      date: string;
