...
export interface UseAlarmThresholdReturn {
  ...
  getAvailablePresets: () => Array<{ id: string; name: string; description?: string; dataType: string }>;
...
export function useAlarmThreshold(options: UseAlarmThresholdOptions = {}): UseAlarmThresholdReturn {
  ...
  const [thresholdHistory, setThresholdHistory] = useState<Array<{ timestamp: number; value: number; triggered: boolean }>>([]);
  ...
  const allThresholds = useMemo((): AlarmThreshold[] => {
    return thresholds.map((threshold, index) => {
      ...
      return {
        ...
        name: threshold.name,
        minValue: threshold.type === 'min' || threshold.type === 'range' ? threshold.value : undefined,
        maxValue: threshold.type === 'max' ? threshold.value : (threshold.type === 'range' ? threshold.maxValue : undefined),
  ...
  const validateThresholdConfig = useCallback((config: Partial<AlarmThreshold>): { valid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    if (config.conditions?.hysteresis !== undefined && (config.conditions.hysteresis < 0 || config.conditions.hysteresis > 100)) {
  ...
  // Threshold management
  const createThreshold = useCallback((config: Partial<AlarmThreshold>): string => {
    ...
    const newThreshold = {
      ...
      type: (config.minValue !== undefined && config.maxValue !== undefined) ? 'range' as const
           : config.minValue !== undefined ? 'min' as const
           : 'max' as const,
  ...
  const updateThreshold = useCallback((updates: Partial<AlarmThreshold>) => {
    if (!threshold) return;
    ...
    const storeUpdate = {
      ...
      type: (updates.minValue !== undefined && updates.maxValue !== undefined) ? 'range' as const
           : updates.minValue !== undefined ? 'min' as const
           : updates.maxValue !== undefined ? 'max' as const
           : undefined,
      value: updates.minValue ?? updates.maxValue,
      maxValue: updates.maxValue,
      level: updates.priority === 'low' ? 'info' as const
           : updates.priority === 'critical' ? 'critical' as const
           : 'warning' as const,
  ...
  }, [threshold, pendingUpdates, validateThresholds, validateThresholdConfig, storeUpdateThreshold, onThresholdUpdated, autoSave, saveDelay, onError]);
  ...
  const duplicateThreshold = useCallback((namePrefix = 'Copy of '): string | null => {
  ...
  const setMinValue = useCallback((value: number | undefined) => {
    updateThreshold({ minValue: value });
  }, [updateThreshold]);

  const setMaxValue = useCallback((value: number | undefined) => {
  ...
  const setPriority = useCallback((priority: AlarmThreshold['priority']) => {
  ...
  const setSoundFile = useCallback((filename: string) => {
  ...
  const testNotifications = useCallback(() => {
    ...
    console.log('Testing notifications for threshold:', threshold.name);
  }, [threshold]);
  ...
  const testThreshold = useCallback((testValue: number): boolean => {
  ...
  const saveAsPreset = useCallback((name: string, description?: string) => {
    ...
    console.log('Saving threshold preset:', { name, description, threshold });
  }, [threshold]);

  const loadPreset = useCallback((presetId: string) => {
    console.log('Loading threshold preset:', presetId);
  }, []);

  const getAvailablePresets = useCallback(() => {
    return [
      { id: 'depth-shallow', name: 'Shallow Water', description: 'Alert when depth < 2m', dataType: 'depth' },
      { id: 'speed-fast', name: 'High Speed', description: 'Alert when speed > 10 knots', dataType: 'speed' },
      { id: 'battery-low', name: 'Low Battery', description: 'Alert when battery < 12V', dataType: 'battery' },
    ];
  }, []);
  ...
  const getThresholdSummary = useCallback(() => {
    ...
    const status = !threshold.enabled ? 'Disabled' 
                 : threshold.isActive ? 'Active' 
                 : 'Normal';
  ...
  const getDataTypeInfo = useCallback((dataType: string) => {
    const dataTypeMap: Record<string, { unit: string; range: { min: number; max: number }; precision: number; label: string }> = {
      ...
      winddirection: { unit: '°', range: { min: 0, max: 360 }, precision: 0, label: 'Wind Direction' },
      battery: { unit: 'V', range: { min: 10, max: 15 }, precision: 1, label: 'Battery Voltage' },
      temperature: { unit: '°C', range: { min: -10, max: 50 }, precision: 1, label: 'Water Temperature' },
      heading: { unit: '°', range: { min: 0, max: 360 }, precision: 0, label: 'Heading' },
    };
    
    return dataTypeMap[dataType] || { unit: '', range: { min: 0, max: 100 }, precision: 0, label: 'Unknown' };
  }, []);

  // Real-time monitoring effect
  useEffect(() => {
    if (!enableRealTimeMonitoring || !threshold || !threshold.enabled || currentValue === undefined) {
      return;
    }
    ...
    // Record history
    setThresholdHistory(prev => [...prev.slice(-99), {
      timestamp: Date.now(),
      value: currentValue,
  ...
  }, [currentValue, withinThreshold, threshold, enableRealTimeMonitoring, updateThreshold, onThresholdTriggered, onThresholdCleared]);
