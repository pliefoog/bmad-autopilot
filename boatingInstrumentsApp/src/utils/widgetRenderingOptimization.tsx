    ...
    (props: P) => {
      ...
      React.useEffect(() => {
        ...
        if (__DEV__ && duration > 16.67) {
          console.warn(
            `[WidgetOptimization] ${widgetType} render took ${duration.toFixed(
              2,
            )}ms (>16.67ms threshold)`,
          );
        }
      });
        ...
        for (const key of keys) {
          ...
          if (typeof prevValue === 'number' && typeof nextValue === 'number') {
            ...
            if (config?.significantChangePercent) {
              const percentChange = Math.abs(((nextValue - prevValue) / (prevValue || 1)) * 100);
...
export function useNativeAnimation(config: {
  ...
  const { duration = 300, useNativeDriver = true, loop = false } = config;
