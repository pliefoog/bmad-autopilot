 ...
 */
export const validateThemePalette = (colors: Record<string, string>): Record<string, ColorCompliance> => {
 ...
 */
export const validateThemeColorsInDev = (themeName: string, colors: Record<string, string> | any): void => {
  ...
  if (__DEV__ && process.env.NODE_ENV === 'development' && process.env.VALIDATE_THEMES === 'true') {
    ...
    if (themeName === 'red-night') {
      ...
      if (violations.length > 0) {
        ...
        console.warn('⚠️  Red-night theme violations can destroy night vision and pose marine safety risks!');
        console.warn('📖 Marine Standard: Red-night mode must emit ZERO blue/green light (620-750nm only)');
      } else {
        console.log(`✅ Red-night theme passes marine night vision compliance`);
      }
    }
