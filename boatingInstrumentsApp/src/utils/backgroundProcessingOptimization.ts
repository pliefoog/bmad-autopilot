...
export function extractFieldIndices(sentence: string, indices: Int32Array): number {
  let fieldCount = 0;
  const currentIndex = sentence.indexOf(',') + 1; // Start after sentence type
...
export class NmeaPriorityQueue {
  ...
  getStats() {
    ...
    for (const [priority, queue] of this.queues.entries()) {
      const name =
        Object.keys(NMEA_PRIORITIES).find(
  ...
  getMetrics(queue?: NmeaPriorityQueue): BackgroundProcessingMetrics {
    ...
    const metrics: BackgroundProcessingMetrics = {
      ...
      averageProcessingTimeMs:
        this.sentenceCount > 0 ? this.totalProcessingTime / this.sentenceCount : 0,
    ...
    metrics.poolUtilization =
      poolStats.maxSize > 0 ? (poolStats.maxSize - poolStats.poolSize) / poolStats.maxSize : 0;

    // Estimate CPU usage (very rough approximation)
    // Assumes processing time / elapsed time = CPU fraction
    metrics.cpuUsagePercent =
      elapsedSeconds > 0 ? Math.min(100, (this.totalProcessingTime / elapsed) * 100) : 0;
...
export function logPerformanceWarnings(metrics: BackgroundProcessingMetrics): void {
  if (__DEV__) {
    ...
    if (metrics.averageProcessingTimeMs > 5) {
      console.warn(
        `[NMEA Performance] Slow processing detected: ${metrics.averageProcessingTimeMs.toFixed(
          2,
        )}ms avg ` + `(peak: ${metrics.peakProcessingTimeMs.toFixed(2)}ms)`,
      );
    }
