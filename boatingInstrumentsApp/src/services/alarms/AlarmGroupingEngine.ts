...
export class AlarmGroupingEngine {
  ...
  public processAlarms(alarms: Alarm[]): AlarmGroup[] {
    ...
    return Array.from(this.activeGroups.values())
      .sort((a, b) => this.compareGroupPriority(a, b));
  }
  ...
  public getCriticalAlarms(alarms: Alarm[]): Alarm[] {
    return alarms.filter(alarm => 
      ...
      this.isCriticalSafetyAlarm(alarm)
    );
  }
    ...
    return [
      ...
      // Electrical System Rules
      {
        ...
        patterns: {
          ...
          dataPathPatterns: ['electrical.batteryVoltage', 'electrical.chargingCurrent', 'electrical.alternatorOutput'],
  ...
  private mergeRelatedGroups(): void {
    ...
    for (const relationship of this.relationships) {
      ...
      const relatedGroups = groups.filter(g => 
        relationship.relatedSystems.includes(g.category)
      );
  ...
  private checkSimultaneousAlarms(
    ...
    for (const relatedGroup of relatedGroups) {
      for (const alarm of relatedGroup.alarms) {
        const hasNearbyPrimary = primaryTimes.some(time =>
          Math.abs(alarm.timestamp - time) <= timeWindow
        );
  ...
  private validateGroupPriorities(): void {
    ...
    for (const group of groups) {
      if (group.category === MarineSystemCategory.SAFETY || 
  ...
  private matchesRelationship(alarm: Alarm, relationship: AlarmRelationship): boolean {
    ...
    const keywordMatch = relationship.keywords.some(keyword =>
      alarm.message.toLowerCase().includes(keyword.toLowerCase()) ||
  ...
  private isCriticalSafetyAlarm(alarm: Alarm): boolean {
    ...
    return criticalSafetySources.some(type =>
      alarm.source?.includes(type) ||
      alarm.message.toLowerCase().includes(type.toLowerCase())
    );
  }
}