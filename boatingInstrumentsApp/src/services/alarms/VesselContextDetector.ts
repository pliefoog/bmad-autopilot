...
export class VesselContextDetector {
  ...
  public getHistoricalData(windowMs?: number): NmeaDataSnapshot[] {
    const cutoff = windowMs 
      ? Date.now() - windowMs 
      : Date.now() - this.config.longTermWindowMs;
  ...
  public getDetectionStats(): {
    ...
    return {
      ...
      dataSpanMs: this.dataHistory.length > 0 
        ? Date.now() - this.dataHistory[0].timestamp 
        : 0,
  ...
  private detectVesselState(data: NmeaDataSnapshot[]): void {
    ...
    const positions = data
      .map(d => d.position)
      .filter(p => p !== undefined) as { latitude: number; longitude: number }[];
    ...
    this.currentContext.state = avgSpeed > this.config.anchoredSpeedThreshold 
      ? 'unknown' 
      : 'anchored';
  }
  
  private isAnchored(avgSpeed: number, maxSpeed: number, positions: { latitude: number; longitude: number }[]): boolean {
    // Speed check
    if (avgSpeed > this.config.anchoredSpeedThreshold || 
  ...
  private isMotoring(data: NmeaDataSnapshot[]): boolean {
    const engineData = data.filter(d => d.engineRunning !== undefined || d.engineRpm !== undefined);
    ...
    const runningCount = engineData.filter(d => 
  ...
  private isSailing(data: NmeaDataSnapshot[], avgSpeed: number): boolean {
    const windData = data.filter(d => 
      d.windSpeed !== undefined || d.trueWindSpeed !== undefined
    );
  ...
  private detectOperatingMode(data: NmeaDataSnapshot[]): void {
    ...
    if (recentEngineData.length > 0) {
      ...
      const avgRpm = recentEngineData
        .map(d => d.engineRpm!)
        .reduce((sum, rpm) => sum + rpm, 0) / recentEngineData.length;
  ...
  private calculateTimeOfDay(): 'day' | 'night' | 'dawn' | 'dusk' {
    ...
    if (timeInMinutes >= sunrise - dawnDuskBuffer && timeInMinutes <= sunrise + dawnDuskBuffer) {
      return 'dawn';
    } else if (timeInMinutes >= sunset - dawnDuskBuffer && timeInMinutes <= sunset + dawnDuskBuffer) {
      return 'dusk';
    } else if (timeInMinutes > sunrise + dawnDuskBuffer && timeInMinutes < sunset - dawnDuskBuffer) {
      return 'day';
    } else {
      return 'night';
    }
  ...
  private calculateConfidence(recentData: NmeaDataSnapshot[]): void {
    ...
    const engineData = recentData.filter(d => 
      d.engineRunning !== undefined || d.engineRpm !== undefined
    );
    if (engineData.length > 0) {
      confidence += 0.1;
    }
    ...
    const windData = recentData.filter(d => 
      d.windSpeed !== undefined || d.trueWindSpeed !== undefined
    );
    if (windData.length > 0) {
      confidence += 0.1;
  ...
  private calculateMovementPattern(): void {
    ...
    const courses = windowData.map(d => d.courseOverGround).filter(c => c !== undefined) as number[];
    ...
    // Time span
    const timeSpan = windowData.length > 0 
