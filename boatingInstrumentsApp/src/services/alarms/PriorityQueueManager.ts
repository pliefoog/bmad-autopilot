...
export interface QueueConfiguration {
  ...
  escalationIntervals: { // milliseconds for each level
...
export class PriorityQueueManager {
  ...
  public getQueueStats(): {
    ...
    const oldestAlarm = allEntries.reduce((oldest, entry) =>
      (!oldest || entry.enqueuedAt < oldest.enqueuedAt) ? entry : oldest,
      undefined as PriorityQueueEntry | undefined
    );
    
    const avgRelevance = allEntries.length > 0
      ? allEntries.reduce((sum, entry) => sum + entry.contextRelevance, 0) / allEntries.length
      : 0;
    
    const escalationsPending = allEntries.filter(entry =>
      entry.nextEscalationAt && entry.nextEscalationAt <= Date.now()
    ).length;
  ...
  private createQueueEntry(alarm: Alarm, group?: AlarmGroup): PriorityQueueEntry {
    ...
    const acknowledgeDeadline = alarm.level === 'critical'
  ...
  private addToQueue(entry: PriorityQueueEntry): void {
    ...
    const insertIndex = targetQueue.findIndex(existing => 
  ...
  private shouldBypassGrouping(alarm: Alarm): boolean {
    ...
    return alarm.level === 'critical' && bypassKeywords.some(keyword =>
      alarm.message.toLowerCase().includes(keyword) ||
  ...
  public calculatePriority(
    ...
    if (context) {
      ...
      if (context.state === 'anchored') {
        ...
        if (alarm.message.toLowerCase().includes('anchor') ||
        ...
        if (alarm.message.toLowerCase().includes('navigation') ||
...
class ContextFilter {
  ...
  private isEngineRelated(alarm: Alarm): boolean {
    ...
    return engineKeywords.some(keyword =>
      alarm.message.toLowerCase().includes(keyword)
    );
  }
  ...
  private isVisualAlarm(alarm: Alarm): boolean {
    ...
    return !alarm.message.toLowerCase().includes('audio') &&
