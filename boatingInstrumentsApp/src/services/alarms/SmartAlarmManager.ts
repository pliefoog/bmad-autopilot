...
export class SmartAlarmManager {
  ...
  constructor(config?: Partial<SmartAlarmConfiguration>) {
    ...
    console.log('SmartAlarmManager: Initialized with smart alarm management', {
  ...
  public recordUserInteraction(
    ...
    if (processedAlarm.smartSuppressed && action === 'escalated') {
      this.learningEngine.recordUserOverride(alarmId, 'force_show', 'User escalated suppressed alarm');
   ...
   */
  public completeMaintenance(maintenanceId: string, actualTime?: number, notes?: string): { success: boolean; message: string } {
    if (!this.config.maintenanceIntegrationEnabled) {
  ...
  public getLearningInsights(): {
    ...
    if (!this.config.adaptiveLearningEnabled) {
      return { stats: { totalInteractions: 0, totalPatterns: 0, totalAdjustments: 0, falsePositiveReduction: 0, patternConfidence: 0, lastAnalysis: 0 }, patterns: [], recommendations: [] };
    }
    ...
    if (stats.falsePositiveReduction > 0.2) {
      recommendations.push(`Smart learning has reduced false alarms by ${Math.round(stats.falsePositiveReduction * 100)}%`);
    }
    
    if (stats.patternConfidence > 0.8 && stats.totalPatterns > 5) {
      recommendations.push(`${stats.totalPatterns} learned patterns are helping optimize alarm management`);
    }
    
    if (stats.totalInteractions > 50 && stats.falsePositiveReduction < 0.1) {
      recommendations.push('Consider reviewing alarm thresholds - learning system may need more training data');
    }
  ...
  private async applySmartFeatures(processed: ProcessedAlarm): Promise<void> {
    ...
    if (this.config.adaptiveLearningEnabled && this.config.smartSuppression) {
      ...
      processed.falseAlarmProbability = suppressionResult.suppress ? suppressionResult.confidence : 0;
    }
    ...
    if (this.config.priorityQueueEnabled && !processed.smartSuppressed) {
      ...
      console.log('SmartAlarmManager: Would enqueue alarm in priority system', {
        alarmId: processed.id,
        priority: processed.smartPriority,
        classification: this.classifyMarineSafety(processed),
      });
    ...
    if (this.config.groupingEnabled && this.config.intelligentGrouping && !processed.smartSuppressed) {
  ...
  private processCriticalAlarmFast(alarm: Alarm, startTime: number): ProcessedAlarm {
    ...
    if (this.config.priorityQueueEnabled) {
      console.log('SmartAlarmManager: Critical alarm bypass - immediate processing', {
        alarmId: processed.id,
        priority: processed.smartPriority,
      });
    }
  ...
  private updateProcessingStats(processed: ProcessedAlarm): void {
    ...
    this.stats.responseTimes[processed.level] = (this.stats.responseTimes[processed.level] + processingTime) / 2;
    
    // Update context confidence
    this.stats.contextConfidence = (this.stats.contextConfidence + processed.vesselContext.confidence) / 2;
  ...
  private startPeriodicTasks(): void {
    ...
    if (this.config.maintenanceIntegrationEnabled) {
      this.maintenanceCheckTimer = setInterval(() => {
        const maintenanceAlarms = this.maintenanceScheduler.checkMaintenanceAlarms(this.currentContext);
  ...
  public cleanup(): void {
    ...
    console.log('SmartAlarmManager: Cleaned up resources');
  }
}