  ...
  generateMarineAlarmMessage: (
    ...
    switch (type) {
      case CriticalAlarmType.SHALLOW_WATER:
        return `⚠️ SHALLOW WATER: ${formatValue(value, 'm')} (limit: ${formatValue(
          threshold,
          'm',
        )})`;
      case CriticalAlarmType.ENGINE_OVERHEAT:
        return `🌡️ ENGINE OVERHEAT: ${formatValue(value, '°C')} (limit: ${formatValue(
          threshold,
          '°C',
        )})`;
      ...
      default:
        return `⚠️ CRITICAL ALARM: ${type} - ${formatValue(value, units)} (threshold: ${formatValue(
          threshold,
          units,
        )})`;
    }
