...
export class AdaptiveLearningEngine {
  ...
  public shouldSuppressAlarm(
    ...
    for (const pattern of matchingPatterns) {
      ...
      if (pattern.confidence > 0.8) {
        bestReason = `Pattern: ${pattern.patternType} (${Math.round(pattern.falsePositiveRate * 100)}% false positive rate)`;
      }
    }
  ...
  public getThresholdAdjustment(
    ...
    const matchingAdjustments = this.adjustments.filter(adj =>
  ...
  public recordUserOverride(
    ...
    const relatedPatterns = this.patterns.filter(pattern =>
      pattern.alarmTypes.includes(alarmId)
    );
    ...
    console.log(`AdaptiveLearningEngine: User override recorded`, {
      alarmId,
      overrideType,
      reason,
      affectedPatterns: relatedPatterns.length,
    });
  ...
  public getLearningStats(): {
    ...
    const avgPatternConfidence = this.patterns.length > 0
  ...
  public async importLearningData(
    ...
    try {
      ...
      const validPatterns = data.patterns.filter(pattern =>
        pattern.confidence >= 0.1 && pattern.occurrenceCount >= 1
      );
      
      // Import adjustments (with safety validation)
      const validAdjustments = data.adjustments.filter(adj =>
        adj.adjustmentFactor >= 0.5 && adj.adjustmentFactor <= 2.0 && // 50%-200% range
        adj.confidence >= 0.1
      );
      ...
      console.log('AdaptiveLearningEngine: Learning data imported', {
        importedPatterns: validPatterns.length,
        importedAdjustments: validAdjustments.length,
        totalPatterns: this.patterns.length,
        totalAdjustments: this.adjustments.length,
      });
  ...
  public async resetLearning(): Promise<void> {
    ...
    console.log('AdaptiveLearningEngine: All learning data reset');
  }
  
  // Private implementation methods
  
  private classifyFalsePositive(userAction: AlarmInteraction['userAction'], responseTime: number): boolean {
    // Quick dismissal or ignore suggests false positive
    if (userAction === 'dismissed' && responseTime < 5000) { // < 5 seconds
      return true;
    }
    ...
    if (userAction === 'acknowledged' && responseTime > 60000) { // > 1 minute
      return true;
    }
  ...
  private matchesContext(patternContext: Partial<VesselContext>, currentContext: VesselContext): boolean {
  ...
  private matchesEnvironmentalData(
    ...
    if (patternConditions.speedRange && currentData.speed !== undefined) {
      if (currentData.speed < patternConditions.speedRange.min ||
    ...
    if (patternConditions.windRange && currentData.windSpeed !== undefined) {
      if (currentData.windSpeed < patternConditions.windRange.min ||
  ...
  private isNavigationAlarm(thresholdId: string): boolean {
    ...
    return navigationKeywords.some(keyword =>
      thresholdId.toLowerCase().includes(keyword)
    );
  }
  ...
  private analyzePatterns(): void {
    ...
    console.log('AdaptiveLearningEngine: Pattern analysis completed', {
      totalPatterns: this.patterns.length,
      totalAdjustments: this.adjustments.length,
  ...
  private analyzeInteractionGroup(patternKey: string, interactions: AlarmInteraction[]): void {
    ...
    if (existingPattern) {
      ...
      existingPattern.suppressionScore = this.calculateSuppressionScore(falsePositiveRate, avgResponseTime);
  ...
  private extractEnvironmentalConditions(interactions: AlarmInteraction[]): LearnedPattern['environmentalConditions'] {
  ...
  private calculatePatternConfidence(interactions: AlarmInteraction[], falsePositiveRate: number): number {
    let confidence = 0;
  ...
  private analyzeThresholdAdjustment(thresholdId: string, interactions: AlarmInteraction[]): void {
    ...
    // Apply safety limits
    const limitedAdjustment = Math.min(
      adjustmentFactor,
      1 + this.config.maxThresholdAdjustment
    );
    ...
    if (existingAdjustment) {
      ...
      const recentInteractions = interactions.filter(i =>
        i.timestamp > existingAdjustment.lastApplied - (7 * 24 * 60 * 60 * 1000)
      );
      ...
      const newAdjustment: ThresholdAdjustment = {
        ...
        reversions: 0,
        confidence: this.calculatePatternConfidence(interactions, falsePositives.length / interactions.length),
  ...
  private mergePatterns(patterns: LearnedPattern[]): LearnedPattern[] {
    ...
    for (const pattern of patterns) {
      const key = `${pattern.alarmTypes.join(',')}:${this.createContextKey(pattern.contextConditions as VesselContext)}`;
  ...
  private cleanOldPatterns(): void {
    ...
    this.patterns = this.patterns.filter(pattern =>
      pattern.lastSeen >= cutoff && pattern.confidence >= 0.1
    );
    
    this.adjustments = this.adjustments.filter(adjustment =>
      adjustment.lastApplied >= cutoff && adjustment.confidence >= 0.1
    );
  }
  ...
  private async loadFromStorage(): Promise<void> {
    try {
      ...
      if (stored) {
        ...
        console.log('AdaptiveLearningEngine: Loaded from storage', {
