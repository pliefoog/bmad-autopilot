...
export class NotificationContentManager {
  ...
  private initializeDefaultTemplates(): void {
    ...
    this.templates.set('shallow_water', {
      ...
      richContent: {
        ...
        expandedBody: 'Current depth: {value}m is below safe threshold of {threshold}m\\n\\nPosition: {position}\\nTime: {timestamp}\\n\\nIMEDIATE ACTION REQUIRED',
        actions: [
          {
            id: 'acknowledge',
            title: 'Acknowledge',
    ...
    this.templates.set('engine_temperature', {
      ...
      richContent: {
        ...
        expandedBody: 'Engine temperature {value}°C exceeds normal operating threshold of {threshold}°C\\n\\nEngine: {engine_name}\\nCoolant Temp: {coolant_temp}°C\\nOil Pressure: {oil_pressure} psi\\n\\nMonitor engine systems closely.',
        actions: [
          {
            id: 'acknowledge',
            title: 'Acknowledge',
  ...
  public generateNotificationContent(
    ...
    if (template.richContent) {
      richContent = {
        expandedTitle: this.processTemplate(template.richContent.expandedTitle || template.title, alarm, vesselContext),
        expandedBody: this.processTemplate(template.richContent.expandedBody || template.bodyTemplate, alarm, vesselContext),
  ...
  private createGenericTemplate(alarm: AlarmNotificationData): NotificationTemplate {
    ...
    return {
      ...
      visual: {
        ...
        color: alarm.level === 'critical' ? '#FF0000' : alarm.level === 'warning' ? '#FFA500' : '#0080FF',
  ...
  private processTemplate(
    ...
    if (vesselContext) {
      if (vesselContext.position) {
        const position = `${vesselContext.position.latitude.toFixed(5)}, ${vesselContext.position.longitude.toFixed(5)}`;
      ...
      processed = processed.replace(/{wind_direction}/g, vesselContext.windDirection?.toFixed(0) || 'N/A');
  ...
  public async handleNotificationAction(
    ...
    try {
      ...
      console.log(`[Notification Content] Action handled: ${actionId}`);
  ...
  private async handleAcknowledge(actionId: string, data: any): Promise<void> {
    ...
    if (alarmId) {
      ...
      console.log(`[Notification Content] Acknowledging alarm: ${alarmId}`);
  ...
  private async handleSnooze(actionId: string, data: any): Promise<void> {
    ...
    console.log(`[Notification Content] Snoozing alarm ${alarmId} for ${duration} minutes`);
  ...
  private async handleNavigate(actionId: string, data: any): Promise<void> {
    ...
    console.log(`[Notification Content] Navigating to: ${target}`);
  ...
  private async handleOpenApp(actionId: string, data: any): Promise<void> {
    console.log('[Notification Content] Opening app from notification');
  ...
  private async handleQuickAction(actionId: string, data: any): Promise<void> {
    console.log(`[Notification Content] Executing quick action: ${actionId}`);
    
    if (actionId.includes('restart_gps')) {
      // TODO: Implement GPS restart
      console.log('[Notification Content] Restarting GPS system');
    }
   ...
   */
  private mapActionToResponse(actionId: string): 'acknowledged' | 'snoozed' | 'dismissed' | 'opened' {
  ...
  public getResponseAnalytics(): {
    ...
    this.history.forEach(entry => {
      if (entry.response) {
        responseTypeDistribution[entry.response] = (responseTypeDistribution[entry.response] || 0) + 1;
      }
      if (entry.action) {
  ...
  public addCustomTemplate(template: NotificationTemplate): void {
    ...
    console.log(`[Notification Content] Added custom template: ${template.id}`);
  }
  ...
  public clearHistory(): void {
    this.history = [];
    console.log('[Notification Content] Notification history cleared');
  }
