  ...
  const renderListView = () => {
    ...
    return (
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        </Text>{ALARM_CATEGORIES.map((category) => (
            ...
            <Text style={[styles.categoryTitle, { color: theme.text }]}>{category.title}</Text>{category.alarms.map((alarm) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => handleNavigateToDetail(alarm.type)}><View style={styles.alarmIcon}>
                    <UniversalIcon name={alarm.iconName} size={28} color={theme.text} />
                  </View><View style={styles.alarmInfo}>
                  ...
                  </View><View style={[styles.statusDot, { backgroundColor: isEnabled ? theme.success : theme.error }]} /><UniversalIcon name="chevron-forward-outline" size={20} color={theme.textSecondary} /></Pressable>
              );
            })}
          </View>
        ))}
        <Pressable
          ...
          onPress={handleResetAll}><UniversalIcon name="refresh-outline" size={20} color={theme.error} /><Text style={[styles.resetButtonText, { color: theme.error }]}>Reset All to Defaults</Text></Pressable>
      </ScrollView>
    );
  };
    ...
    return (
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        {config.enabled && config.audioEnabled && (
          ...
          >
            <UniversalIcon name="volume-high-outline" size={24} color={theme.background} /><Text style={[styles.testButtonText, { color: theme.background }]}>Test Alarm Sound</Text>
          </Pressable>
        )}
      </ScrollView>
    );
