...
class CIResourceOptimizer {
  ...
  calculateOptimalWorkers() {
    ...
    console.log(
      `🧮 Calculated optimal Jest workers: ${optimalWorkers} (CPUs: ${availableCPUs}, Memory: ${memoryGB.toFixed(
        1,
      )}GB)`,
    );
  ...
  checkResourceViolations(memoryMetrics, cpuMetrics, diskMetrics) {
    ...
    if (memoryMetrics.utilization > this.config.monitoring.alertThreshold) {
      violations.push({
        ...
        message: `Memory utilization ${(memoryMetrics.utilization * 100).toFixed(
          1,
        )}% exceeds threshold`,
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log(
            '  CI_AUTO_KILL_ON_LIMIT - Auto-kill on critical violations (default: false)',
          );
          process.exit(1);
      }
    } catch (error) {
