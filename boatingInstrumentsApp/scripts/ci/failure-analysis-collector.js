...
class FailureAnalysisCollector {
  ...
  async setupOutputDirectory() {
    ...
    this.config.output.baseDir = path.join(__dirname, `debug-artifacts${timestamp}`);
  ...
  findSuspiciousStackPatterns(stackTraces) {
    ...
    const suspiciousPatterns = [
      {
        pattern: /node_modules\/(?!@react-native|react-native)/,
  ...
  determineLikelyRootCause() {
    ...
    if (
      this.analysisResults.failureTypes.includes('network') ||
      this.analysisResults.failureTypes.includes('websocket')
    ) {
  ...
  async generateRecommendations() {
    ...
    recommendations.push({
      ...
      priority: 'medium',
      description:
        'Check if the failure is related to insufficient system resources (memory, CPU, disk space).',
      action:
        'Monitor resource usage during test execution and consider scaling up CI environment if needed.',
    });
      ...
      timeout: [
        {
          ...
          action:
            'Increase Jest timeout configuration or review async operations for proper cleanup.',
        },
      ],
      ...
      network: [
        {
          ...
          action:
            'Verify external services are running and accessible, or mock network dependencies.',
        },
      ],
      ...
      nmea: [
        {
          ...
          action:
            'Check simulator startup scripts and port configurations. Ensure simulator is properly initialized before tests.',
        },
      ],
  ...
  async packageArtifacts() {
    try {
      ...
      execSync(
        ...
        )}" "${path.basename(this.config.output.baseDir)}"`,
        {
          timeout: 30000,
        },
      );
      ...
      console.log(
        `  📦 Created debug archive: ${archivePath} (${(stats.size / (1024 * 1024)).toFixed(
          2,
        )} MB)`,
      );
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log(
            '  analyze [trigger] [context]  - Analyze failure and collect debug artifacts (default)',
          );
          console.log('  config                       - Show configuration');
          console.log('');
          console.log('Environment Variables:');
          console.log(
            '  CI_MAX_LOG_SIZE_MB          - Maximum log file size to collect (default: 50)',
          );
          ...
          console.log(
            '  CI_ENABLE_PATTERN_MATCHING  - Enable failure pattern analysis (default: true)',
          );
          console.log(
            '  CI_GENERATE_RECOMMENDATIONS - Generate troubleshooting recommendations (default: true)',
          );
          console.log(
            '  CI_COMPRESS_ARTIFACTS       - Create compressed artifact archive (default: true)',
          );
          process.exit(1);
      }
    } catch (error) {
