...
class SelectiveTestRunner {
  ...
  loadDependencyMapping() {
    try {
      if (fs.existsSync(this.config.dependencies.mappingFile)) {
        ...
        console.log(`📋 Loaded dependency mapping with ${Object.keys(mapping.testToSource || {}).length} test mappings`);
        return mapping;
      }
    } catch (error) {
  ...
  findTestFiles() {
    ...
    for (const testDir of this.config.tests.testDirs) {
      try {
        ...
        const command = `find ${globPattern} -type f \\( ${this.config.tests.testExtensions.map(ext => `-name "*${ext}"`).join(' -o ')} \\) 2>/dev/null || true`;
  ...
  analyzeTestDependencies(testFile) {
    ...
    try {
      ...
      const relatedFiles = this.findRelatedFiles(testDir, path.basename(testFile, path.extname(testFile)));
  ...
  getChangedFiles() {
    ...
    try {
      ...
      this.changedFiles = result.trim().split('\n').filter(f => f.length > 0);
  ...
  selectTestsForChangedFiles(changedFiles) {
    ...
    const forceFullRun = changedFiles.some(file => 
      this.config.tests.forceFullTestPatterns.some(pattern => 
        file.includes(pattern.replace('**/*', '')) || 
        (pattern.includes('*') && new RegExp(pattern.replace(/\*\*/g, '.*').replace(/\*/g, '[^/]*')).test(file))
      )
    );
    ...
    console.log(`  ✅ Selected ${this.selectedTests.length} tests in ${this.metrics.analysisTime}ms`);
      ...
      for (const testDir of testDirs) {
        ...
        for (const pattern of testPatterns) {
          try {
            ...
            const foundTests = result.trim().split('\n').filter(f => f.length > 0);
  ...
  async runSelectedTests(selectedTests, options = {}) {
    ...
    if (this.metrics.estimatedSavings < this.config.optimization.minTestSavings) {
      console.log(`⚠️ Savings below threshold (${this.config.optimization.minTestSavings * 100}%), running full test suite`);
      return await this.runFullTests(options);
    }
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log('  CI_AUTO_GENERATE_MAPPING    - Auto-generate dependency mapping (default: true)');
