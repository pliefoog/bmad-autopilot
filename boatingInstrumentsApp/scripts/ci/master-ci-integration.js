  ...
  async initializeComponents() {
    ...
    try {
      ...
      if (this.config.pipeline.enableResourceOptimization) {
        this.components.resourceOptimizer = new CIResourceOptimizer(this.config.components.optimization);
    ...
    try {
      ...
      if (this.config.pipeline.enableSelectiveTesting && !options.forceFullTests) {
        ...
        if (this.components.selectiveRunner) {
          ...
          try {
            ...
            const selectedTests = this.components.selectiveRunner.selectTestsForChangedFiles(changedFiles);
            ...
            console.warn(`  ⚠️ Selective testing failed: ${error.message}, falling back to full tests`);
          }
        }
      }
      ...
      if (this.config.pipeline.enableFlakyDetection && this.components.flakyDetector) {
        if (!testResult.success && testResult.failedTests) {
          ...
          const flakyResult = await this.components.flakyDetector.analyzeAndRetry(testResult.failedTests);
...
if (require.main === module) {
  ...
  if (['full', 'test-only', 'quality-check', 'setup', 'cleanup', 'status'].includes(operation)) {
    ...
    console.log('  CI_ENABLE_SIMULATOR        - Enable NMEA Bridge Simulator management (default: true)');
