...
const sentences = [
  ...
  '$IIMWD,54.5,T,54.5,M,13.0,N,6.7,M*52' // Wind direction/speed
];

console.log('NMEA Parser Analysis:');
console.log('===================');
...
sentences.forEach(sentence => {
  try {
    ...
    console.log(`✓ ${sentence.substring(0, 35).padEnd(40)} → ${parsed.sentenceId.padEnd(5)} Fields: ${fields.slice(0,4).join(', ')}`);
  } catch (e) {
    console.log(`✗ ${sentence.substring(0, 35).padEnd(40)} → ERROR: ${e.message}`);
  }
});