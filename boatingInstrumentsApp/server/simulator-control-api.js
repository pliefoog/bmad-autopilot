...
class SimulatorControlAPI {
  ...
  setupRoutes() {
    ...
    this.app.post('/api/inject-data', (req, res) => {
      try {
        ...
        if (!nmeaSentence) {
          return res
            .status(400)
            .json({ error: 'NMEA sentence is required (use "sentence" or "data" field)' });
        }
    ...
    this.app.post('/api/simulate-error', (req, res) => {
      try {
        ...
        console.log(
          `💥 Simulating error: ${normalizedType} (${type}) for ${duration}ms${
    ...
    this.app.post('/api/story/validate', async (req, res) => {
      try {
        ...
        const finalCriteria =
          acceptance_criteria.length > 0 ? acceptance_criteria : acceptanceCriteria;
        ...
        const validationResult = await this.validateStoryScenario(
          finalStoryId,
          finalScenario,
          finalCriteria,
        );
  ...
  getScenarioCategory(scenarioName) {
    ...
    if (scenarioName.includes('engine') || scenarioName.includes('monitoring'))
      return 'development';
    if (
      scenarioName.includes('alarm') ||
      scenarioName.includes('safety') ||
      scenarioName.includes('battery') ||
      scenarioName.includes('shallow')
    )
      return 'safety';
    if (
      scenarioName.includes('performance') ||
      scenarioName.includes('stress') ||
      scenarioName.includes('frequency') ||
      scenarioName.includes('malformed')
    )
      return 'performance';
    if (
      scenarioName.includes('recorded') ||
      scenarioName.includes('real-world') ||
      scenarioName.includes('synthetic')
    )
      return 'recorded';
    if (scenarioName.includes('story-') || scenarioName.includes('validation'))
  ...
  simulateConnectionLoss(duration, protocols) {
    console.log(
      `💥 Simulating connection loss for ${duration}ms on protocols: ${protocols.join(', ')}`,
    );
