/**
 * BMAD Agent Integration API Server
 * 
 * Provides REST API endpoints for BMAD agents (Dev, QA, Architect) to control
...
class BMADIntegrationAPI {
  constructor(simulator) {
    this.simulator = simulator;
    this.scenarioEngine = new ScenarioEngine();
    this.app = express();
    this.server = null;
    this.apiPort = 9090; // Dedicated port for BMAD API
  ...
  setupMiddleware() {
    ...
    this.app.use((req, res, next) => {
      console.log(`📡 BMAD API: ${req.method} ${req.path} - ${req.ip}`);
      next();
    });
    
    // Error handling middleware
    this.app.use((err, req, res, next) => {
      console.error('❌ BMAD API Error:', err.message);
      res.status(500).json({ 
        error: err.message,
  ...
  /**
   * Setup REST API routes for BMAD agents
   */
  setupRoutes() {
  ...
  /**
   * Start the BMAD Integration API server
   */
  async start(port = this.apiPort) {
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(port, '0.0.0.0', () => {
        console.log(`🔌 BMAD Integration API listening on http://0.0.0.0:${port}`);
        console.log(`📖 API Documentation: http://localhost:${port}/api/health`);
        resolve();
      });
      
      this.server.on('error', (err) => {
        console.error('❌ BMAD API server error:', err.message);
        reject(err);
      });
    });
  }
  ...
  async stop() {
    if (this.server) {
      this.server.close();
      console.log('🛑 BMAD Integration API stopped');
    }
  }
...
module.exports = { BMADIntegrationAPI };