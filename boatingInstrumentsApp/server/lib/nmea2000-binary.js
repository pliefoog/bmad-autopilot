...
class NMEA2000BinaryGenerator {
  ...
  generatePGN_129029(sensor) {
    ...
    const secondsSinceMidnight =
      (now.getUTCHours() * 3600 + now.getUTCMinutes() * 60 + now.getUTCSeconds()) * 10000;
  ...
  generatePGN_130311(sensor) {
    ...
    if (humidityRaw >= 0 && humidityRaw <= 25000) {
      // 0-100% range
      data.writeUInt16LE(humidityRaw, 4);
    } else {
  ...
  generatePGN_127489(sensor) {
    ...
    if (sensor.data_generation?.coolant_temperature) {
      coolantTemp = this.getYAMLDataValue(
        'coolant_temperature',
        sensor.data_generation.coolant_temperature,
      );
    }

    if (sensor.data_generation?.alternator_voltage) {
      alternatorVoltage = this.getYAMLDataValue(
        'alternator_voltage',
