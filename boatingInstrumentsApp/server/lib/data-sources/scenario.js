...
const SENSOR_TYPE_REGISTRY = {
  depth_sensor: {
    ...
    physical_properties: {
      transducer_depth: {
        unit: 'm',
        description: 'Depth of transducer below waterline',
        nmea0183_field: 'DPT.offset',
        nmea2000_field: 'byte[8]',
      },
      ...
      mounting_location: {
        type: 'string',
        description: 'Physical mounting position (bow/stern/center)',
      },
    },
  },
  speed_sensor: {
    ...
    physical_properties: {
      sensor_type: {
        type: 'enum',
        values: ['paddle_wheel', 'electromagnetic', 'pitot_tube'],
        description: 'Speed sensor technology',
      },
      mounting_location: { type: 'string', description: 'Hull mounting position' },
      calibration_factor: {
        unit: 'ratio',
        description: 'Speed calibration multiplier',
        nmea0183_field: 'applied_to_knots',
        nmea2000_field: 'byte[2-3]',
      },
    },
  },
  wind_sensor: {
    ...
    physical_properties: {
      ...
      reference_type: {
        type: 'enum',
        values: ['apparent', 'true', 'ground'],
        description: 'Wind reference frame',
        nmea0183_field: 'MWV.reference',
        nmea2000_field: 'byte[5].bits[0-2]',
      },
    },
  },
  gps_sensor: {
    ...
    physical_properties: {
      ...
      horizontal_accuracy: {
        unit: 'm',
        description: 'Expected GPS accuracy',
        nmea0183_field: 'GGA.hdop',
        nmea2000_field: 'byte[14-15]',
      },
  ...
  heading_sensor: {
    ...
    physical_properties: {
      sensor_type: {
        type: 'enum',
        values: ['magnetic_compass', 'fluxgate', 'gyro', 'gps_derived'],
        description: 'Heading sensor technology',
      },
      deviation: {
        unit: 'degrees',
        description: 'Magnetic deviation',
        nmea0183_field: 'HDG.deviation',
        nmea2000_field: 'byte[4-5]',
      },
      variation: {
        unit: 'degrees',
        description: 'Magnetic variation',
        nmea0183_field: 'HDG.variation',
        nmea2000_field: 'byte[6-7]',
      },
  ...
  temperature_sensor: {
    ...
    physical_properties: {
      sensor_location: {
        type: 'enum',
        values: ['sea_water', 'engine_room', 'cabin', 'refrigeration', 'exhaust', 'oil'],
        description: 'Temperature sensor location',
        nmea0183_field: 'XDR.transducer_name',
        nmea2000_field: 'byte[4]',
      },
      sensor_depth: { unit: 'm', description: 'Depth of water temperature sensor' },
      calibration_offset: {
        unit: '°C',
        description: 'Temperature calibration offset',
        nmea0183_field: 'applied_to_celsius',
        nmea2000_field: 'applied_to_kelvin',
      },
    },
  },
  ...
  engine_sensor: {
    ...
    physical_properties: {
      engine_instance: {
        type: 'integer',
        description: 'Engine number (0=port, 1=starboard)',
        nmea0183_field: 'RPM.source',
        nmea2000_field: 'byte[0]',
      },
      rpm_calibration: { unit: 'ratio', description: 'RPM calibration factor' },
      sensor_types: {
        type: 'array',
        description:
          'Available sensors: rpm, coolant_temp, oil_pressure, alternator_voltage, fuel_rate, engine_hours',
      },
    },
  },
  battery_sensor: {
    ...
    physical_properties: {
      battery_instance: {
        type: 'integer',
        description: 'Battery bank number',
        nmea0183_field: 'XDR.transducer_name',
        nmea2000_field: 'byte[0]',
      },
  ...
  tank_sensor: {
    ...
    physical_properties: {
      tank_instance: {
        ...
        nmea0183_field: 'derived',
        nmea2000_field: 'byte[0]',
      },
      fluid_type: {
        type: 'enum',
        values: ['fuel', 'fresh_water', 'gray_water', 'black_water', 'oil'],
        description: 'Tank contents',
        nmea0183_field: 'none',
        nmea2000_field: 'byte[1]',
      },
      capacity: { unit: 'L', description: 'Total tank capacity', nmea2000_field: 'byte[2-5]' },
      sensor_type: {
        type: 'enum',
        values: ['resistive', 'capacitive', 'ultrasonic'],
        description: 'Level sensor technology',
      },
    },
  },
  rudder_sensor: {
    ...
    physical_properties: {
      rudder_instance: {
        type: 'integer',
        description: 'Rudder number (0=main)',
        nmea0183_field: 'RSA.position',
        nmea2000_field: 'byte[0]',
      },
  ...
  autopilot: {
    ...
    physical_properties: {
      control_type: {
        type: 'enum',
        values: ['compass', 'gps', 'wind'],
        description: 'Autopilot control mode',
      },
...
class ScenarioDataSource extends EventEmitter {
  ...
  startGenerator(name, generator, phase) {
    ...
    if (name.includes('vhw') || name.includes('vtg')) {
      console.log(
        `⏱️ Starting timer for ${name}: baseInterval=${baseInterval}ms, speed=${this.config.speed}, actualInterval=${interval}ms`,
      );
    }
  ...
  startContinuousGeneration() {
    ...
    if (this.hasYAMLScenario) {
      console.log(
        `📋 YAML scenario loaded - built-in generators disabled (scenario defines all data sources)`,
      );
    }
    ...
    builtInGenerators.forEach((generatorName) => {
      ...
      if (
        generatorName === 'depth' &&
        (yamlGeneratorTypes.has('dpt') ||
          yamlGeneratorTypes.has('dbt') ||
          yamlGeneratorTypes.has('dbk') ||
          sensorGeneratorTypes.has('depth_sensor'))
      ) {
      ...
      if (
        ...
        (yamlGeneratorTypes.has('vhw') ||
          ...
          sensorGeneratorTypes.has('speed_sensor'))
      ) {
      ...
      if (
        generatorName === 'wind' &&
        (yamlGeneratorTypes.has('mwv') || sensorGeneratorTypes.has('wind_sensor'))
      ) {
      ...
      if (
        ...
        (yamlGeneratorTypes.has('gga') ||
          ...
          sensorGeneratorTypes.has('gps_sensor'))
      ) {
      ...
      if (generator) {
        console.log(
          `▶️ Starting built-in generator: ${generatorName} (no YAML scenario - legacy mode)`,
        );
  ...
  initializeYAMLGenerators() {
    ...
    if (!sentences || !Array.isArray(sentences)) {
      console.log(
        `⚠️ No YAML sentences found in scenario (expected 'sensors' or 'nmea_sentences' array)`,
      );
      return;
    }
    ...
    sentences.forEach((sentenceDef, index) => {
      ...
      switch (sentenceDef.type) {
        ...
        case 'DBK':
          ...
          console.log(
            `📏 Added YAML ${sentenceDef.type} depth generator: ${generatorName} at ${frequencyHz}Hz`,
          );
          break;
        ...
        case 'MTW':
          ...
          console.log(
            `🌡️ Added YAML MTW water temp generator: ${generatorName} at ${frequencyHz}Hz`,
          );
          break;
        ...
        case 'VHW':
          ...
          console.log(
            `⛵ Added YAML VHW water speed generator: ${generatorName} at ${frequencyHz}Hz (interval: ${intervalMs}ms)`,
          );
          break;
        ...
        case 'VTG':
          ...
          console.log(
            `🧭 Added YAML VTG track generator: ${generatorName} at ${frequencyHz}Hz (interval: ${intervalMs}ms)`,
          );
          break;
        ...
        case 'HDM':
          ...
          console.log(
            `🧭 Added YAML ${sentenceDef.type} heading generator: ${generatorName} at ${frequencyHz}Hz`,
          );
          break;
        ...
        case 'PCDIN':
          ...
          console.log(
            `🔧 Added YAML PCDIN (NMEA 2000) generator: ${generatorName} at ${frequencyHz}Hz (PGN ${sentenceDef.pgn})`,
          );
          break;
  ...
  initializeSensorGenerators(sensors) {
    ...
    sensors.forEach((sensor, index) => {
      ...
      if (sensor.type === 'speed_sensor') {
        console.log(
          `🔍 Speed sensor found: instance=${sensor.instance}, speed_type=${sensor.physical_properties?.speed_type}, name="${sensor.name}"`,
        );
      }
      ...
      console.log(
        `📡 Added sensor generator: ${generatorName} (${sensor.type}) at ${updateRateHz}Hz in ${bridgeMode} mode`,
      );
    });
  }
  ...
  generateNMEA0183FromSensor(sensor, sensorType) {
    ...
    if (sensor.type === 'temperature_sensor') {
      const locationCode =
        sensor.physical_properties?.location || sensor.physical_properties?.sensor_type || '';
      const isWaterTemp =
        locationCode.toUpperCase() === 'SEAW' ||
        locationCode === 'sea_water' ||
        locationCode === 'water' ||
        sensor.physical_properties?.sensor_type === 'water';
      const isAirTemp =
        locationCode.toUpperCase() === 'AIRX' ||
        locationCode === 'air' ||
  ...
  generateYAMLBatterySentence(sentenceDef) {
    ...
    sentenceDef.instances.forEach((instanceDef) => {
      ...
      const voltage = batteryData.voltage
        ? this.getYAMLDataValue('voltage', batteryData.voltage)
        : null;
      ...
      if (voltage !== null) {
        ...
        const current = batteryData.current
          ? this.getYAMLDataValue('current', batteryData.current)
          : null;
        const temperature = batteryData.temperature
          ? this.getYAMLDataValue('temperature', batteryData.temperature)
          : null;
        const soc = batteryData.state_of_charge
          ? this.getYAMLDataValue('state_of_charge', batteryData.state_of_charge)
          : null;
        ...
        if (current !== null) {
          ...
          messages.push(
            currentSentence + '*' + this.calculateChecksum(currentSentence.substring(1)),
          );
        }
        ...
        console.log(
          `🔋 Generated battery ${instance} (${batteryName}): V=${voltage.toFixed(
            2,
          )}V, I=${current?.toFixed(2)}A, SOC=${soc?.toFixed(0)}%, T=${temperature?.toFixed(1)}°C`,
        );
      }
    });
  ...
  generateYAMLTankSentence(sentenceDef) {
    ...
    sentenceDef.instances.forEach((instanceDef) => {
      ...
      if (level !== null) {
        ...
        console.log(
          `🪣 Generated tank ${tankId}: ${(level * 100).toFixed(1)}% (${level.toFixed(3)} ratio)`,
        );
      }
    });
  ...
  generateYAMLTemperatureSentence(sentenceDef) {
    ...
    sentenceDef.instances.forEach((instanceDef) => {
      ...
      if (temperature !== null) {
        ...
        console.log(
          `🌡️ Generated temperature ${tempId} (${locationName}): ${temperature.toFixed(1)}°C`,
        );
      }
    });
  ...
  generateYAMLRPMSentence(sentenceDef) {
    ...
    if (sentenceDef.sentence && sentenceDef.parameters) {
      const sentenceWithParams = this.replaceSentenceParameters(
        sentenceDef.sentence,
        sentenceDef.parameters,
      );
  ...
  generateTidalCycle(config, currentTime) {
    ...
    return Math.max(config.min_depth || 0, Math.min(config.max_depth || 100, depth));
  }
  ...
  generateCoastalWind(config, currentTime) {
    ...
    return Math.max(config.min || 0, Math.min(config.max || 50, gustSpeed));
  }
  ...
  generatePolarSailing(config, currentTime) {
    ...
    return Math.max(config.min || 0, Math.min(config.max || 15, speed));
  }
  ...
  generateSpeedFromSensor(sensor, sentenceType) {
    ...
    if (speedType === 'SOG' || sentenceType === 'VTG') {
      ...
      console.log(
        `🚤 VTG (instance ${instance}): SOG=${speedKnots.toFixed(
          2,
        )} knots, heading=${heading.toFixed(1)}°`,
      );
  ...
  generateGPSFromSensor(sensor, sentenceTypes) {
    ...
    if (
      sensor.data_generation?.longitude &&
    ...
    if (sentenceTypes.includes('VTG') && speedKnots !== null) {
      ...
      const sentence = `VTG,${cog.toFixed(1)},T,${magneticTrack},M,${speedKnots.toFixed(
        1,
      )},N,${speedKmh},K,A`;
      const checksum = this.calculateChecksum(sentence);
      // Changed from GP to II for NKE Display Pro compatibility (like DPT fix)
      sentences.push(
        `$IIVTG,${cog.toFixed(1)},T,${magneticTrack},M,${speedKnots.toFixed(
          1,
        )},N,${speedKmh},K,A*${checksum}`,
      );
    }
    ...
    if (sentences.length > 0 && speedKnots !== null) {
      console.log(
        `🛰️ GPS: ${sentences.length} sentences, SOG=${speedKnots.toFixed(
          2,
        )} knots, COG=${cog.toFixed(1)}°, HDG=${heading.toFixed(1)}°`,
      );
    }
  ...
  generateTemperatureFromSensor(sensor, sentenceType) {
    ...
    const locationCode =
      sensor.physical_properties?.location || sensor.physical_properties?.sensor_type || 'TEMP';
    const instance = sensor.instance || 0;
  ...
  generateEngineFromSensor(sensor, sentenceType) {
    ...
    if (sensorTypes.includes('alternator_voltage') && sensor.data_generation?.alternator_voltage) {
      const voltage = this.getYAMLDataValue(
        'alternator_voltage',
        sensor.data_generation.alternator_voltage,
      );
  ...
  generateBatteryFromSensor(sensor, sentenceType) {
    ...
    if (sensor.data_generation?.state_of_charge) {
      ...
      if (nominalVoltage === 12) {
        ...
        soc = Math.max(
          0,
          Math.min(100, ((voltage - nominalVoltage * 0.9) / (nominalVoltage * 0.2)) * 100),
        );
      }
    }
    ...
    if (xdrFormat === 'compound') {
      ...
      const sentence = `IIXDR,U,${voltage.toFixed(2)},V,${batteryId},I,${current.toFixed(
        2,
      )},A,${batteryId},C,${tempCelsius.toFixed(1)},C,${batteryId},P,${soc.toFixed(
        1,
      )},%,${batteryId},U,${nominalVoltage.toFixed(1)},V,${batteryId}_NOM,V,${capacity.toFixed(
        0,
      )},H,${batteryId},G,${chemistry},N,${batteryId}`;
  ...
  generateTankFromSensor(sensor, sentenceType) {
    ...
    const fluidType =
      sensor.physical_properties?.tank_type || sensor.physical_properties?.fluid_type || 'fuel';
    const capacity =
      sensor.physical_properties?.tank_capacity || sensor.physical_properties?.capacity || 200;
  ...
  generateDBT(depth) {
    ...
    const checksum = this.calculateChecksum(
      `DBT,${depthFeet},f,${depthMeters},M,${depthFathoms},F`,
    );
    // Changed from GP to II for NKE Display Pro compatibility
  ...
  generateVLW(speedKnots, instance = 0) {
    ...
    if (Math.random() < 0.05) {
      console.log(
        `📊 VLW (instance ${instance}): Total=${totalNM}nm, Trip=${tripNM}nm, STW=${speedKnots.toFixed(
          2,
        )}kts`,
      );
    }
  ...
  generateRMC(lat, lon, speed = null, track = null) {
    ...
    const dateStr =
      time.toISOString().substr(8, 2) +
      time.toISOString().substr(5, 2) +
      time.toISOString().substr(2, 2);
    ...
    const sentence = `RMC,${timeStr},A,${Math.floor(latDeg)}${latMin},${latDir},${Math.floor(
      lonDeg,
    )}${lonMin},${lonDir},${speedKnots},${trackDegrees},${dateStr},${variationAbs},${variationDir}`;
    const checksum = this.calculateChecksum(sentence);
    // Changed from GP to II for NKE Display Pro compatibility (like DPT fix)
    return `$II${sentence}*${checksum}`;
  }
  ...
  generateGLL(lat, lon) {
    ...
    const sentence = `GLL,${Math.floor(latDeg)}${latMin},${latDir},${Math.floor(
      lonDeg,
    )}${lonMin},${lonDir},${timeStr},A,A`;
    const checksum = this.calculateChecksum(sentence);
  ...
  generateAPB() {
    ...
    // Build APB sentence
    const sentence = `APB,${status},${lockStatus},${crossTrackError.toFixed(
      2,
    )},${xteDirection},${arrivalStatus},${waypointPassed},${waypointPassed},${bearingToWaypoint.toFixed(
      1,
    )},M,${waypointId},${bearingToWaypoint.toFixed(1)},M,${currentHeading.toFixed(1)},M`;
    const checksum = this.calculateChecksum(sentence);
    // Changed from GP to II for NKE Display Pro compatibility
    return `$II${sentence}*${checksum}`;
  }
  ...
  generateDPT(depth, transducerDepth = null, rangeScale = null) {
    ...
    const offset =
      transducerDepth !== null
        ? transducerDepth
        : this.scenario.parameters?.vessel?.keel_offset || 0;
    // Range scale: User-selected display range (usually not reported, leave empty)
    const scale =
      rangeScale !== null ? rangeScale : this.scenario.parameters?.sonar?.range_scale || '';
    const scaleField = scale !== '' ? `,${scale.toFixed(1)}` : ',';
    const checksum = this.calculateChecksum(
      `DPT,${depth.toFixed(1)},${offset.toFixed(1)}${scaleField}`,
    );
  ...
  generateDBK(depth) {
    ...
    const checksum = this.calculateChecksum(
      `DBK,${depthFeet},f,${depthMeters},M,${depthFathoms},F`,
    );
    // Changed from SD to II for NKE Display Pro compatibility (integrated instrumentation talker ID)
  ...
  generateMMBSentence() {
    const pressureConfig =
      this.scenario?.data?.atmospheric_pressure || this.scenario?.data?.pressure;
  ...
  generateVTGSentence() {
    ...
    console.log(
      `🚤 VTG called - sogConfig: ${!!sogConfig}, stwConfig: ${!!stwConfig}, gpsConfig: ${!!gpsConfig}`,
    );
    ...
    if (sogConfig) {
      if (sogConfig.type === 'gps_track' || sogConfig.source === 'gps_track') {
        ...
        speed = stw + current;
        console.log(
          `🚤 VTG: SOG from STW + current: ${stw.toFixed(2)} + ${current.toFixed(
            2,
    ...
    const checksum = this.calculateChecksum(
      `VTG,${heading.toFixed(1)},T,${magneticTrack},M,${speed.toFixed(1)},N,${speedKmh},K,A`,
    );
    // Changed from GP to II for NKE Display Pro compatibility (like DPT fix)
    return [
      `$IIVTG,${heading.toFixed(1)},T,${magneticTrack},M,${speed.toFixed(
        1,
      )},N,${speedKmh},K,A*${checksum}`,
    ];
  }
  ...
  generateGGASentence(latitude = null, longitude = null, sensor = null) {
    ...
    const sentence = `GGA,${timeStr},${Math.floor(latDeg)}${latMin},${latDir},${String(
      Math.floor(lonDeg),
    ).padStart(3, '0')}${lonMin},${lonDir},${quality},${String(satellites).padStart(
      2,
      '0',
    )},${hdop},${altitude},M,0.0,M,,`;
    const checksum = this.calculateChecksum(sentence);
    // Changed from GP to II for NKE Display Pro compatibility
    return [`$II${sentence}*${checksum}`];
  }
  ...
  getSpeedFromTrack() {
    ...
    const gpsSensor = this.scenario.sensors?.find(
      (s) => s.type === 'gps_sensor' && s.data_generation?.position?.waypoints,
    );
  ...
  getCurrentCOG(smooth = true) {
    ...
    const gpsSensor = this.scenario?.sensors?.find(
      (sensor) =>
        sensor.type === 'gps_sensor' && sensor.data_generation?.position?.type === 'gps_track',
    );
  ...
  calculateCrossTrackError() {
    ...
    const x1C =
      Math.cos(lat1) * Math.sin(latC) - Math.sin(lat1) * Math.cos(latC) * Math.cos(dLon1C);
    ...
    const a =
      Math.sin(dLat / 2) * Math.sin(dLat / 2) +
      Math.cos(lat1) * Math.cos(latC) * Math.sin(dLon / 2) * Math.sin(dLon / 2);
  ...
  generatePCDIN_127488(sensorOrSentenceDef) {
    ...
    const data = `PCDIN,01F200,${this.toHex(instance)},${this.toHex(rpmLow)},${this.toHex(
      rpmHigh,
    )},FF,FF,FF,FF,FF`;
  ...
  generatePCDIN_127508(sensorOrSentenceDef) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01F214,${this.toHex(instance)},${this.toHex(voltLow)},${this.toHex(
      voltHigh,
    )},${this.toHex(currLow)},${this.toHex(currHigh)},${this.toHex(tempLow)},${this.toHex(
      tempHigh,
    )},FF`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_127505(sensorOrSentenceDef) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01F211,${this.toHex(instance)},${this.toHex(fluidType)},${this.toHex(
      levelLow,
    )},${this.toHex(levelHigh)},${this.toHex(capB0)},${this.toHex(capB1)},${this.toHex(
      capB2,
    )},${this.toHex(capB3)}`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_128267(sensor) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01F503,FF,${this.toHex(depthB0)},${this.toHex(depthB1)},${this.toHex(
      depthB2,
    )},${this.toHex(depthB3)},${this.toHex(offsetB0)},${this.toHex(offsetB1)},FF`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_130306(sensor) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01FD02,FF,${this.toHex(speedB0)},${this.toHex(speedB1)},${this.toHex(
      angleB0,
    )},${this.toHex(angleB1)},${this.toHex(reference)}`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_129029(sensor) {
    ...
    const data = `PCDIN,01F805,00,FF,FF,FF,FF,FF,FF,${this.toHex(latB0)},${this.toHex(
      latB1,
    )},${this.toHex(latB2)},${this.toHex(latB3)},${this.toHex(lonB0)},${this.toHex(
      lonB1,
    )},${this.toHex(lonB2)},${this.toHex(lonB3)}`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_127250(sensor) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01F112,FF,${this.toHex(headingB0)},${this.toHex(headingB1)},${this.toHex(
      deviationB0,
    )},${this.toHex(deviationB1)},${this.toHex(variationB0)},${this.toHex(variationB1)}`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_130310(sensor) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01FD06,FF,${this.toHex(tempSource)},${this.toHex(tempB0)},${this.toHex(
      tempB1,
    )},FF,FF`;
    const checksum = this.calculateChecksum(data);
  ...
  generatePCDIN_127245(sensor) {
    ...
    // Build PCDIN sentence
    const data = `PCDIN,01F0FD,${this.toHex(instance)},${this.toHex(angleB0)},${this.toHex(
      angleB1,
    )}`;
    const checksum = this.calculateChecksum(data);
