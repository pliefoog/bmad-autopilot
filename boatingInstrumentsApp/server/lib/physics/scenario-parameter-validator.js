...
class ScenarioParameterValidator {
  ...
  async validateScenario(scenarioConfig, scenarioName = 'unknown') {
    ...
    if (scenarioConfig.parameters?.physics?.enabled) {
      const physicsResult = await this._validatePhysicsConfiguration(scenarioConfig, scenarioName);
  ...
  async _validatePhysicsConfiguration(scenarioConfig, scenarioName) {
    ...
    try {
      ...
      if (physicsConfig.overrides) {
        result.profile = this.profileManager.applyOverrides(
          result.profile,
  ...
  _validateParameterRelationships(scenarioConfig, profile) {
    ...
    if (physics.overrides?.behavioral) {
      ...
      if (behavioral.crew_skill_level && behavioral.risk_tolerance !== undefined) {
        ...
        if (
          expectedRange &&
          (behavioral.risk_tolerance < expectedRange[0] ||
            behavioral.risk_tolerance > expectedRange[1])
        ) {
          result.warnings.push({
    ...
    if (physics.overrides?.behavioral && physics.overrides?.physics) {
      ...
      if (physicsParams.max_heel && behavioral.sailing_mode) {
        ...
        if (
          expectedRange &&
          (physicsParams.max_heel < expectedRange[0] || physicsParams.max_heel > expectedRange[1])
        ) {
          result.warnings.push({
    ...
    if (validation.fuel_consumption_range && profile.engine) {
      ...
      if (profile.engine.fuel_flow_cruise && maxConsumption > profile.engine.fuel_flow_cruise * 2) {
        result.warnings.push({
          ...
          value: {
            target_max: maxConsumption,
            cruise_consumption: profile.engine.fuel_flow_cruise,
          },
        });
      }
    }
  ...
  _computeDerivedParameters(profile, scenarioConfig) {
    ...
    if (profile.dimensions?.beam && profile.dimensions?.displacement) {
      computed.stability_index =
        profile.dimensions.beam / Math.sqrt(profile.dimensions.displacement / 64);
    }

    // Compute power-to-weight ratio for powerboats
    if (
      profile.type === 'powerboat' &&
      profile.engine?.total_horsepower &&
      profile.dimensions?.displacement
    ) {
      computed.power_to_weight =
        profile.engine.total_horsepower / (profile.dimensions.displacement / 2240);
    }

    // Compute sail area to displacement ratio for sailboats
    if (
      profile.type === 'sailboat' &&
      profile.dimensions?.sail_area &&
      profile.dimensions?.displacement
    ) {
      computed.sail_area_displacement_ratio =
        profile.dimensions.sail_area / Math.pow(profile.dimensions.displacement / 64, 2 / 3);
    }
