...
class PolarDiagramProcessor {
    ...
    async loadPolarDiagram(polarFilePath) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!fs.existsSync(polarFilePath)) {
                reject(new Error(`Failed to read polar file: ENOENT: no such file or directory, open '${polarFilePath}'`));
                return;
            }
    ...
    _parsePolarData(rows, polarData) {
        ...
        const angleColumns = Object.keys(firstRow).filter(key => 
            key !== 'TWS' && key !== 'tws' && !isNaN(parseFloat(key))
        );
        
        polarData.windAngles = angleColumns
    ...
    interpolateSpeed(polarData, trueWindSpeed, trueWindAngle) {
        ...
        const result = this._bilinearInterpolation(
            polarData,
            trueWindSpeed,
            normalizedAngle
        );
    ...
    _bilinearInterpolation(polarData, windSpeed, windAngle) {
        ...
        const speedRatio = (windSpeed - windSpeeds[speedIndex1]) / 
                          (windSpeeds[speedIndex2] - windSpeeds[speedIndex1]);
    ...
    _linearInterpolateAngle(polarData, windSpeed, windAngle, speedIndex) {
        ...
        const angleRatio = (windAngle - windAngles[angleIndex1]) / 
                          (windAngles[angleIndex2] - windAngles[angleIndex1]);
