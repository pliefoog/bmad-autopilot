...
class VesselDynamics {
    ...
    _updateApparentWind() {
        ...
        this.state.apparentWindSpeed = Math.sqrt(apparentWindX * apparentWindX + apparentWindY * apparentWindY) / 0.514444;
    ...
    _calculateTargetSpeed() {
        ...
        const baseSpeed = this._estimateSpeedFromWind(this.state.trueWindSpeed, this.state.trueWindAngle);
        return baseSpeed * this.environmentalEffects.speedReduction;
    }
    ...
    _updateHeading(deltaTime, targetHeading) {
        ...
        const maxTurnRate = this.physics.maxTurnRate * speedFactor * this.environmentalEffects.turnRateFactor;
    ...
    _updatePosition(deltaTime) {
        ...
        const vesselLonDelta = speedMs * Math.sin(headingRad) * deltaTime / (60 * Math.cos(this.state.latitude * Math.PI / 180));
        
        // Current effect
        const currentRad = this.state.currentDirection * Math.PI / 180;
        const currentLatDelta = currentMs * Math.cos(currentRad) * deltaTime / 60;
        const currentLonDelta = currentMs * Math.sin(currentRad) * deltaTime / (60 * Math.cos(this.state.latitude * Math.PI / 180));
...
class CoordinatedVesselState {
    ...
    _updatePositionWithCourseIntegration(physicsState, timestamp) {
        ...
        if (deltaTimeSeconds > 0 && deltaTimeSeconds < 60) { // Reasonable time delta
            const speedKnots = physicsState.speed || 0;
            const headingDegrees = physicsState.heading || 0;
            
            if (speedKnots > 0.1) { // Only integrate if moving
    ...
    reset() {
        ...
        this.vesselState.motion = { 
            speed: 0, speedOverGround: 0, heading: 0, courseOverGround: 0, 
            acceleration: 0, turnRate: 0, timestamp: now 
        };
        this.vesselState.wind = { 
            trueSpeed: 0, trueAngle: 0, apparentSpeed: 0, apparentAngle: 0, timestamp: now 
        };
        this.vesselState.environment = { 
            currentSpeed: 0, currentDirection: 0, waveHeight: 0, seaState: 0, temperature: 20, timestamp: now 
        };
        this.vesselState.control = { 
            rudderAngle: 0, engineRpm: 0, throttlePosition: 0, sailTrim: 0, timestamp: now 
        };
    ...
    update(deltaTime, targetState) {
        ...
        if (typeof targetState.targetSpeed === 'number') {
            ...
            const speedChange = Math.sign(speedDiff) * Math.min(
                Math.abs(speedDiff),
                this.speedTransitionRate * deltaTime
            );
...
class WindController extends StateController {
    ...
    update(deltaTime, targetState) {
        ...
        if (typeof targetState.trueWindSpeed === 'number') {
            ...
            const speedChange = Math.sign(speedDiff) * Math.min(
                Math.abs(speedDiff),
                this.windSpeedTransitionRate * deltaTime
            );
        ...
        if (typeof targetState.trueWindAngle === 'number') {
            ...
            const angleChange = Math.sign(angleDiff) * Math.min(
                Math.abs(angleDiff),
                this.windAngleTransitionRate * deltaTime
            );
...
class EnvironmentController extends StateController {
    ...
    update(deltaTime, targetState) {
        ...
        if (typeof targetState.currentSpeed === 'number') {
            ...
            const speedChange = Math.sign(speedDiff) * Math.min(
                Math.abs(speedDiff),
                this.currentTransitionRate * deltaTime
            );
        ...
        if (typeof targetState.waveHeight === 'number') {
            ...
            const heightChange = Math.sign(heightDiff) * Math.min(
                Math.abs(heightDiff),
                this.waveTransitionRate * deltaTime
            );
...
class ControlController extends StateController {
    ...
    update(deltaTime, targetState) {
        ...
        if (typeof targetState.rudderAngle === 'number') {
            ...
            const angleChange = Math.sign(angleDiff) * Math.min(
                Math.abs(angleDiff),
                this.rudderTransitionRate * deltaTime
            );
        ...
        if (typeof targetState.throttlePosition === 'number') {
            ...
            const throttleChange = Math.sign(throttleDiff) * Math.min(
                Math.abs(throttleDiff),
                this.throttleTransitionRate * deltaTime
            );
...
class SynchronizedNMEAGenerator {
    ...
    generateSynchronizedSentences(options = {}) {
        ...
        Object.entries(this.sentenceTimings).forEach(([sentenceType, timing]) => {
            if (this._shouldGenerateSentence(sentenceType, currentTime, timing)) {
                const sentence = this._generateSentenceForType(sentenceType, vesselStateSnapshot, currentTime);
                if (sentence) {
                    sentences.push({
                        type: sentenceType,
                        sentence: sentence,
    ...
    _generateSpeedSentences(state) {
        ...
        const vtgSentence = `$IIVTG,${cog.toFixed(1)},T,,M,${sog.toFixed(1)},N,${(sog * 1.852).toFixed(1)},K,A`;
        
        // VHW - Water Speed and Heading
        const vhwSentence = `$IIVHW,${heading.toFixed(1)},T,,M,${stw.toFixed(1)},N,${(stw * 1.852).toFixed(1)},K`;
        
        return [
            this._addChecksum(vtgSentence),
            this._addChecksum(vhwSentence)
        ];
    }
    ...
    _generateWindSentence(state) {
        ...
        const sentence = `$IIMWV,${windAngle.toFixed(1)},${windDir},${apparentWindSpeed.toFixed(1)},N,A`;
        return this._addChecksum(sentence);
    }
    ...
    _generateGPSSentences(state, timestamp) {
        ...
        const dateStr = `${now.getUTCDate().toString().padStart(2, '0')}${(now.getUTCMonth() + 1).toString().padStart(2, '0')}${now.getUTCFullYear().toString().slice(-2)}`;
        ...
        const rmcSentence = `$IIRMC,${timeStr},A,${latStr},${latHem},${lonStr},${lonHem},${sog.toFixed(1)},${cog.toFixed(1)},${dateStr},,,A`;
        
        return [
            this._addChecksum(ggaSentence),
            this._addChecksum(rmcSentence)
        ];
    }
    ...
    _generateAutopilotSentence(state) {
        ...
        const sentence = `$IAPB,A,A,0.00,R,N,V,V,${heading.toFixed(1)},M,DEST,${heading.toFixed(1)},M,${heading.toFixed(1)},M`;
        return this._addChecksum(sentence);
    }
