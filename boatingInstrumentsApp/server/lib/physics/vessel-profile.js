...
class VesselProfileManager {
    ...
    async loadProfile(profileName) {
        ...
        if (!fs.existsSync(profilePath)) {
            throw new Error(`Vessel profile not found: ${profileName}. Available profiles: ${this.getAvailableProfiles().join(', ')}`);
        }
    ...
    _validateProfile(profile, profileName) {
        ...
        if (errors.length > 0) {
            throw new Error(`Vessel profile validation failed for '${profileName}':\n${errors.join('\n')}`);
        }
    ...
    _validateDimensions(dimensions) {
        ...
        if (!dimensions.length_overall || dimensions.length_overall < 10 || dimensions.length_overall > 200) {
    ...
    _validatePerformanceParameters(performance, vesselType) {
        ...
        if (vesselType === 'powerboat') {
            ...
            if (performance.cruise_speed && (performance.cruise_speed < 5 || performance.cruise_speed > 50)) {
    ...
    _addComputedProperties(profile) {
        ...
        if (profile.type === 'sailboat' && profile.dimensions.ballast && profile.dimensions.displacement) {
