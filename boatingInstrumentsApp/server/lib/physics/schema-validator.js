...
class ScenarioSchemaValidator {
            ...
            if (phase.events) {
                phase.events.forEach((event, eventIndex) => {
                    const physicsCommands = [
                        'set_wind_conditions', 'adjust_sail_trim', 'initiate_tack',
                        'set_engine_power', 'optimize_pointing', 'physics_reset'
                    ];
    ...
    _validatePhysicsDataSources(data, scenarioName) {
        ...
        const physicsDataTypes = [
            'physics_calculated', 'polar_interpolation', 'apparent_wind_calculation',
            'vessel_state_integration', 'sailing_dynamics', 'engine_power_curve'
        ];
