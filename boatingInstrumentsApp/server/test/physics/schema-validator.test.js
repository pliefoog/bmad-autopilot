...
describe('ScenarioSchemaValidator', () => {
  ...
  describe('Physics Configuration Validation', () => {
    ...
    test('should reject physics scenario without vessel profile', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((e) => e.message.includes('vessel_profile is required'))).toBe(
        true,
      );
    });
    ...
    test('should reject scenario with non-existent vessel profile', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((e) => e.message.includes('Vessel profile file not found'))).toBe(
        true,
      );
    });
    ...
    test('should generate warnings for questionable parameter combinations', () => {
      ...
      expect(result.warnings.some((w) => w.message.includes('unrealistic behavior'))).toBe(true);
    });
  });
  ...
  describe('File Validation', () => {
    ...
    test('should handle malformed YAML files', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some((e) => e.message.includes('Failed to load scenario file'))).toBe(
        true,
      );
    });
