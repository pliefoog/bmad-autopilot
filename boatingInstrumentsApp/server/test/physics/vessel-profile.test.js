...
describe('VesselProfileManager', () => {
    ...
    describe('Profile Loading', () => {
        ...
        test('should throw error for non-existent profile', async () => {
            await expect(profileManager.loadProfile('non-existent'))
                .rejects.toThrow('Vessel profile not found: non-existent');
        });
        ...
        test('should throw error for malformed YAML', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('malformed'))
                    .rejects.toThrow('Failed to load vessel profile');
            } finally {
                // Clean up
    ...
    describe('Profile Validation', () => {
        test('should validate required fields', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('invalid-test'))
        ...
        test('should validate vessel type', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('invalid-type-test'))
                    .rejects.toThrow('Invalid vessel type');
            } finally {
                if (fs.existsSync(tempPath)) {
                    fs.unlinkSync(tempPath);
                }
        ...
        test('should validate dimension ranges', async () => {
            ...
            const invalidProfile = {
                ...
                physics: { keel_offset: 0.5, leeway_factor: 5, heel_sensitivity: 2, vmg_efficiency: 0.8, max_heel: 20, stall_angle: 45 }
            };
            
            fs.writeFileSync(tempPath, require('js-yaml').dump(invalidProfile));
            
            try {
                await expect(profileManager.loadProfile('invalid-dimensions-test'))
        ...
        test('should validate physics parameter ranges', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('invalid-physics-test'))
                    .rejects.toThrow('must be between');
            } finally {
                if (fs.existsSync(tempPath)) {
                    fs.unlinkSync(tempPath);
        ...
        test('should validate sailboat-specific requirements', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('invalid-sailboat-test'))
                    .rejects.toThrow('must specify a polar_diagram');
            } finally {
                if (fs.existsSync(tempPath)) {
                    fs.unlinkSync(tempPath);
                }
        ...
        test('should validate powerboat-specific requirements', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('invalid-powerboat-test'))
                    .rejects.toThrow('must specify cruise_speed');
            } finally {
                if (fs.existsSync(tempPath)) {
                    fs.unlinkSync(tempPath);
                }
    ...
    describe('Error Handling', () => {
        ...
        test('should handle file system errors gracefully', async () => {
            ...
            try {
                await expect(profileManager.loadProfile('j35'))
                    .rejects.toThrow('Failed to load vessel profile');
            } finally {
