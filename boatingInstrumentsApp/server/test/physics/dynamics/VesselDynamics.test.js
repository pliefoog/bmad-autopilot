...
const { VesselDynamics, CoordinatedVesselState, SynchronizedNMEAGenerator } = require('../../../lib/physics/dynamics/VesselDynamics');
...
describe('VesselDynamics', () => {
    ...
    describe('utility functions', () => {
        ...
        test('should calculate bearing correctly', () => {
            const bearing = VesselDynamics.calculateBearing(
                37.7749, -122.4194, // San Francisco
                37.8044, -122.2711  // Oakland (northeast)
            );
    ...
    describe('integration scenarios', () => {
        test('should handle complete sailing scenario', () => {
            ...
            for (let i = 0; i < 30; i++) { // 30 x 10 second updates
                dynamics.updateState(10, {}, {
                    trueWindSpeed: 15,
                    trueWindAngle: 45,
                    waveHeight: 1.0,
                    currentSpeed: 0.5,
...
describe('CoordinatedVesselState', () => {
    ...
    describe('state transition controllers', () => {
        ...
        test('should apply smooth heading transitions with wrap-around', () => {
            ...
            const finalDistance = Math.min(Math.abs(10 - state.motion.heading), 360 - Math.abs(10 - state.motion.heading)); 
...
describe('SynchronizedNMEAGenerator', () => {
    ...
    describe('NMEA sentence generation', () => {
        ...
        test('should generate valid GPS sentences', () => {
            ...
            // GGA sentence
            expect(sentences[0]).toMatch(/^\$IIGGA,[\d.]+,[\d.]+,[NS],[\d.]+,[EW],1,08,[\d.]+,[\d.]+,M,[\d.]+,M,,\*[0-9A-F]{2}$/);
            
            // RMC sentence
            expect(sentences[1]).toMatch(/^\$IIRMC,[\d.]+,A,[\d.]+,[NS],[\d.]+,[EW],[\d.]+,[\d.]+,\d+,,,A\*[0-9A-F]{2}$/);
        });
        ...
        test('should generate valid autopilot sentence', () => {
            ...
            expect(sentence).toMatch(/^\$IAPB,A,A,[\d.]+,R,N,V,V,[\d.]+,M,DEST,[\d.]+,M,[\d.]+,M\*[0-9A-F]{2}$/);
        });
    });
    ...
    describe('position tracking with course integration', () => {
        test('should update position based on speed and heading', () => {
            ...
            const positionChanged = (state.position.latitude !== initialLat || 
                                   state.position.longitude !== initialLon);
        ...
        test('should apply current effects to position', () => {
            ...
            coordState.updateState(3600.0, { // 1 hour
                targetSpeed: 5.0,
                targetHeading: 0,
                currentSpeed: 2.0,
                currentDirection: 90
