...
class ScenarioEngine {
  ...
  async loadScenario(scenarioPath, config = {}) {
    try {
      ...
      console.log(
        `📋 Scenario loaded: ${scenario.name} (${scenarioId}) - Category: ${scenario.category}`,
      );
  ...
  listScenarios() {
    try {
      ...
      console.log(
        `📋 Found ${availableScenarios.length} scenarios across ${categories.length} categories`,
      );
  ...
  async validateStory(storyId, acceptanceCriteria) {
    try {
      ...
      console.log(
        `✅ Story validation completed: ${storyId} (${validation.passed} passed, ${validation.failed} failed)`,
      );
      return validation;
    } catch (error) {
