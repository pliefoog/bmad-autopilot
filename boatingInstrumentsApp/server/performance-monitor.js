...
class BMADPerformanceMonitor {
  ...
  checkPerformanceAlerts(metrics) {
    ...
    if (cpuPercent > this.alerting.maxCpuPercent) {
      alerts.push({
        type: 'cpu_high',
        severity: 'warning',
        message: `CPU usage exceeded threshold: ${cpuPercent.toFixed(1)}% > ${
          this.alerting.maxCpuPercent
        }%`,
        value: cpuPercent,
    ...
    // Message rate alert
    if (
      metrics.messagesPerSecond < this.alerting.minMessagesPerSecond &&
    ...
    if (metrics.avgResponseTime > this.alerting.maxResponseTimeMs) {
      alerts.push({
        ...
        severity: 'warning',
        message: `Response time exceeded threshold: ${metrics.avgResponseTime.toFixed(1)}ms > ${
          this.alerting.maxResponseTimeMs
        }ms`,
  ...
  checkPerformanceRegression(metrics) {
    ...
    const memoryIncrease =
      (metrics.memoryHeapUsed - this.baseline.avgMemoryHeapUsed) / this.baseline.avgMemoryHeapUsed;
    if (memoryIncrease > this.regressionThreshold) {
      alerts.push({
        type: 'memory_regression',
        severity: 'critical',
        message: `Memory usage regression detected: ${(memoryIncrease * 100).toFixed(
          1,
    ...
    const messageDecrease =
      (this.baseline.avgMessagesPerSecond - metrics.messagesPerSecond) /
      this.baseline.avgMessagesPerSecond;
    if (messageDecrease > this.regressionThreshold && metrics.messagesPerSecond > 0) {
      alerts.push({
        type: 'throughput_regression',
        severity: 'critical',
        message: `Message throughput regression detected: ${(messageDecrease * 100).toFixed(
          1,
    ...
    if (this.memorySnapshots.length >= 5) {
      ...
      if (trend.isIncreasing && trend.rate > 1024 * 1024) {
        ...
        this.processAlert({
          ...
          severity: 'critical',
          message: `Memory leak detected: ${(trend.rate / 1024 / 1024).toFixed(
            1,
          )}MB/minute increase`,
          trend: trend,
          timestamp: timestamp,
